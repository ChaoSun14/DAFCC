...
Performance_timing_unit::Performance_timing_unit(int comp_id, int unit_type, int unit_behavior, int unit_int_keyword, const char *unit_char_keyword)
{
    ...
    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, unit_char_keyword != NULL, "the keyword (last paremeter of the interface) of a performance timing unit for computation task can not be NULL");
    if (unit_type == TIMING_TYPE_COMPUTATION || unit_type == TIMING_TYPE_COMMUNICATION || unit_type == TIMING_TYPE_MODEL_RUN)
...
void Performance_timing_unit::check_timing_unit(int unit_type, int unit_behavior, int unit_int_keyword, const char *unit_char_keyword)
{
    ...
    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, unit_type == TIMING_TYPE_COMMUNICATION || unit_type == TIMING_TYPE_IO || unit_type == TIMING_TYPE_COMPUTATION || unit_type == TIMING_TYPE_MODEL_RUN,
                     "C-Coupler error in checking unit_type in match_timing_unit");
    if (unit_type == TIMING_TYPE_COMMUNICATION)
        EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, unit_behavior == TIMING_COMMUNICATION_RECV_WAIT || unit_behavior == TIMING_COMMUNICATION_SEND_WAIT || unit_behavior == TIMING_COMMUNICATION_SENDRECV ||
    ...
    if (unit_type == TIMING_TYPE_IO)
        EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, unit_behavior == TIMING_IO_INPUT || unit_behavior == TIMING_IO_OUTPUT || unit_behavior == TIMING_IO_RESTART,
                         "C-Coupler error in checking unit_behavior in match_timing_unit");
	if (unit_type == TIMING_TYPE_MODEL_RUN)
        EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, unit_behavior == TIMING_MODEL_RUN, "C-Coupler error in checking unit_behavior in match_timing_unit");		
}
...
bool Performance_timing_unit::match_timing_unit(int unit_type, int unit_behavior, int unit_int_keyword, const char *unit_char_keyword)
{
    ...
    if (unit_type == TIMING_TYPE_COMPUTATION) {
        EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, unit_char_keyword != NULL, "C-Coupler software error in match_timing_unit of Performance_timing_unit: unit_char_keyword is NULL");
...
{
    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, previous_time == -1.0, "C-Coupler or model error in starting performance timing: timing unit (%s) has not been stoped", unit_char_keyword);
    wtime(&previous_time);
}
...
void Performance_timing_unit::timing_stop()
{
    ...
    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, previous_time != -1.0, "C-Coupler or model error in stopping performance timing: timing unit has not been started");
    wtime(&current_time);
...
//            printf("%s spends %lf seconds for data communication for data remapping in each process on average\n", compset_communicators_info_mgr->get_current_comp_name(), all_process_sum_time/num_procs);
    ...
    else if (unit_type == TIMING_TYPE_MODEL_RUN)
        EXECUTION_REPORT(REPORT_CONSTANTLY, comp_id, true, "the component model \"%s\" spends %lf seconds for model run \"%s\" at the current process\n", comp_comm_group_mgt_mgr->get_global_node_of_local_comp(comp_id,false,"")->get_full_name(), total_time, unit_char_keyword);
}
