...
class Comp_comm_group_mgt_node
{
    ...
    public:
		...
		void add_child(Comp_comm_group_mgt_node *child) { children.push_back(child); }
		bool is_given_comp_a_child(Comp_comm_group_mgt_node *);
		int get_num_members_in_ensemble() { return num_members_in_ensemble; }
		int set_num_members_in_ensemble(int number) { num_members_in_ensemble = number; }
};
...
class Comp_comm_group_mgt_mgr
{
    private:
		...
		std::vector<const char*> temp_comps_name;
		...
		char external_procedure_lib_dir[NAME_STR_SIZE];
		char external_procedure_config_dir[NAME_STR_SIZE];
        char ensemble_procedure_config_dir[NAME_STR_SIZE];
    ...
    public:
		...
		int register_ensemble_set_of_comps(const char *, int, int, const char *);
		void add_temp_comp_name(const char *);
		void clear_temp_comps_name();
		...
		const char *get_external_procedure_lib_dir() const { return external_procedure_lib_dir; }
		const char *get_external_procedure_config_dir() const { return external_procedure_config_dir; }
        const char *get_ensemble_procedure_config_dir() const { return ensemble_procedure_config_dir; }
		...
		int get_num_members_in_ensemble_set(Comp_comm_group_mgt_node *, Comp_comm_group_mgt_node *);	
		void load_comp_info_of_ensemble(std::vector<Comp_comm_group_mgt_node *> &, int, const char *, int, MPI_Comm, int);
};


extern void create_directory(const char *, MPI_Comm, bool, bool, bool);
