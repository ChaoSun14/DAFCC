...
Decomp_info *Decomp_info_mgt::generate_remap_weights_src_decomp(Decomp_info *dst_decomp_info, Original_grid_info *src_original_grid, Original_grid_info *dst_original_grid, Remap_weight_of_strategy_class *remap_weights)
{
    ...
    if (decomp_for_remap == NULL) {
        decomp_for_remap = new Decomp_info(decomp_name_remap, (TYPE_DECOMP_ID_PREFIX|decomps_info.size()), dst_original_grid->get_comp_id(), src_H2D_original_grid_id, num_local_cells, local_cell_global_indexes, 0, NULL, "in Decomp_info_mgt::generate_remap_weights_src_decomp", false);
...
int Decomp_info_mgt::register_H2D_parallel_decomposition(const char *decomp_name, int grid_id, int num_local_cells, const int *cell_indexes_in_decomp, int num_chunks, const int *chunks_size, const char *annotation)
{
    Decomp_info *new_decomp = new Decomp_info(decomp_name, (TYPE_DECOMP_ID_PREFIX|decomps_info.size()), -1, grid_id, num_local_cells, cell_indexes_in_decomp, num_chunks, chunks_size, annotation, true);
...
Decomp_info *Decomp_info_mgt::generate_default_parallel_decomp(Original_grid_info *original_grid)
{
	...
	existing_decomp_info = get_decomp_info(register_H2D_parallel_decomposition(default_parallel_decomp_name, H2D_sub_grid->get_grid_id(), num_local_cells, local_cells_global_index, 0, NULL, "in Decomp_info_mgt::generate_default_parallel_decomp"));
