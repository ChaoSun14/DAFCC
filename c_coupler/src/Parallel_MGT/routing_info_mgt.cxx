...
Routing_info *Routing_info_mgt::search_or_add_router(const int src_comp_id, const int dst_comp_id, const char *src_decomp_name, const char *dst_decomp_name)
{
    ...
    if (router != NULL) {
		EXECUTION_REPORT_LOG(REPORT_LOG, src_comp_id, true, "The router from (%s %s) to (%s %s) already exists", comp_comm_group_mgt_mgr->search_global_node(src_comp_id)->get_full_name(), src_decomp_name, comp_comm_group_mgt_mgr->search_global_node(src_comp_id)->get_full_name(), dst_decomp_name);
        return router;
    }
...
Routing_info_with_one_process *Routing_info::generate_routing_info_between_procs(routing_mapping_table_entry* local_routing_mapping_table_entries, int num_local_routing_mapping_table_entries, int remote_proc_global_id, const int *local_cells_chunk_id, bool is_src)
{
	...
	for (int i = 0; i < num_local_routing_mapping_table_entries; i ++) {
		if (last_reference_local_index + 1 != local_routing_mapping_table_entries[i].local_index || local_cells_chunk_id != NULL && local_cells_chunk_id[last_reference_local_index] != local_cells_chunk_id[local_routing_mapping_table_entries[i].local_index])
                                                  ...
                                                  routing_mapping_table_entry* local_routing_mapping_table_entries, int routing_mapping_table_entries_count)
{
    ...
    for (j = 0; j < num_reference_cells; j ++) 
        if (reference_cell_indx[j] != CCPL_NULL_INT && logical_indx_lookup_table_local[reference_cell_indx[j]] != -1 && logical_indx_lookup_table_remote[reference_cell_indx[j]] != -1) {
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, local_routing_mapping_table_entries[routing_info->num_elements_transferred].global_index == reference_cell_indx[j] && j == local_routing_mapping_table_entries[routing_info->num_elements_transferred].key, "ERROR happens in Routing_info::compute_routing_info_between_decomps: different index between router info: %d %d, %d %d\n", local_routing_mapping_table_entries[routing_info->num_elements_transferred].global_index, local_routing_mapping_table_entries[routing_info->num_elements_transferred].key, reference_cell_indx[j], j);
