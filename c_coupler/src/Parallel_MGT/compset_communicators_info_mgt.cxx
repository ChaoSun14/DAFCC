...
void create_directory(const char *path, MPI_Comm comm, bool is_root_proc, bool new_dir, bool barrier)
{
	...
	if (barrier) {
		EXECUTION_REPORT(REPORT_ERROR, -1, comm != MPI_COMM_NULL, "Software error in create_directory");
	    MPI_Barrier(comm);
	}
}
...
*/
    ...
    if (ancestor != NULL) {
        if (is_real_component_model()) {
            ...
            create_directory(working_dir, comm_group, get_current_proc_local_id() == 0, false, false);
            sprintf(working_dir, "%s/CCPL_dir/run/data/%s/%s", comp_comm_group_mgt_mgr->get_root_working_dir(), comp_type, full_name);
            create_directory(working_dir, comm_group, get_current_proc_local_id() == 0, false, false);
			sprintf(dir, "%s/CCPL_dir/run/data/%s/%s/restart", comp_comm_group_mgt_mgr->get_root_working_dir(), comp_type, full_name);
            create_directory(dir, comm_group, get_current_proc_local_id() == 0, false, false);
        }
        ...
        create_directory(dir, comm_group, get_current_proc_local_id() == 0, false, true);
...
int Comp_comm_group_mgt_node::get_local_proc_global_id(int local_indx)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, -1, false, "Software error in Comp_comm_group_mgt_node::get_local_proc_global_id: %d vs %d", local_indx, local_processes_global_ids.size());
...
bool Comp_comm_group_mgt_node::is_given_comp_a_child(Comp_comm_group_mgt_node *comp_node)
{
	for (int i = 0; i < children.size(); i ++)
		if (children[i] == comp_node)
			return true;
...
Comp_comm_group_mgt_mgr::Comp_comm_group_mgt_mgr(const char *executable_name)
{
    ...
    create_directory(temp_string, MPI_COMM_WORLD, current_proc_global_id == 0, false, false);
    sprintf(temp_string, "%s/CCPL_dir/run/CCPL_logs", root_working_dir);
    create_directory(temp_string, MPI_COMM_WORLD, current_proc_global_id == 0, true, false);
    ...
    create_directory(temp_string, MPI_COMM_WORLD, current_proc_global_id == 0, false, false);
	sprintf(external_procedure_lib_dir, "%s/CCPL_dir/libs/external_procedures/", root_working_dir);
	sprintf(external_procedure_config_dir, "%s/CCPL_dir/config/all/external_procedures/", root_working_dir);
    sprintf(ensemble_procedure_config_dir, "%s/CCPL_dir/config/all/ensemble_procedures/", root_working_dir);
    ...
    create_directory(internal_H2D_grids_dir, MPI_COMM_WORLD, current_proc_global_id == 0, true, false);
    sprintf(internal_remapping_weights_dir, "%s/CCPL_dir/run/data/all/internal_remapping_weights", root_working_dir);
    create_directory(internal_remapping_weights_dir, MPI_COMM_WORLD, current_proc_global_id == 0, false, false);
    sprintf(components_processes_dir, "%s/CCPL_dir/run/data/all/components_processes", root_working_dir);
    create_directory(components_processes_dir, MPI_COMM_WORLD, current_proc_global_id == 0, true, false);
    sprintf(components_exports_dir, "%s/CCPL_dir/run/data/all/components_exports", root_working_dir);
    create_directory(components_exports_dir, MPI_COMM_WORLD, current_proc_global_id == 0, true, false);
    sprintf(active_coupling_connections_dir, "%s/CCPL_dir/run/data/all/active_coupling_connections", root_working_dir);
    create_directory(active_coupling_connections_dir, MPI_COMM_WORLD, current_proc_global_id == 0, true, false);    
    sprintf(comps_ending_config_status_dir, "%s/CCPL_dir/run/data/all/comps_ending_config_status", root_working_dir);
    create_directory(comps_ending_config_status_dir, MPI_COMM_WORLD, current_proc_global_id == 0, true, false);
    sprintf(restart_common_dir, "%s/CCPL_dir/run/data/all/restart", root_working_dir);
    create_directory(restart_common_dir, MPI_COMM_WORLD, current_proc_global_id == 0, false, false);
    ...
    create_directory(temp_string, MPI_COMM_WORLD, current_proc_global_id == 0, false, true);    
    ...
    if (proc_id == 0) {
        ...
        for (int j = 0; j < unique_executable_name.size(); j ++) {
            ...
            create_directory(dir, MPI_COMM_WORLD, true, false, false);
        }
...
Comp_comm_group_mgt_mgr::~Comp_comm_group_mgt_mgr()
{
	...
	clear_temp_comps_name();
}


void Comp_comm_group_mgt_mgr::clear_temp_comps_name()
{
	for (int i = 0; i < temp_comps_name.size(); i ++)
		delete temp_comps_name[i];
	
	temp_comps_name.clear();
}


void Comp_comm_group_mgt_mgr::add_temp_comp_name(const char *comp_name)
{	
	temp_comps_name.push_back(strdup(comp_name));
}


int Comp_comm_group_mgt_mgr::register_ensemble_set_of_comps(const char *comps_ensemble_set_name, int member_comp_id, int ensemble_size, const char *annotation)
{
	Comp_comm_group_mgt_node *member_comp_node = comp_comm_group_mgt_mgr->search_global_node(member_comp_id);
	int num_comps = temp_comps_name.size(), num_total_procs, count, *all_procs_global_ids;
	std::vector < Comp_comm_group_mgt_node * > all_comp_nodes, temp_comp_nodes;
	MPI_Group global_group, new_set_comm_group;
	MPI_Comm new_set_comm;

	
    synchronize_comp_processes_for_API(member_comp_id, API_ID_COMP_MGT_REG_ENSEMBLE_SET, member_comp_node->get_comm_group(), "synchorization for registering an ensemble set of component models", annotation);	
	// check parent_ensemble_set_id is an ensemble set registered by normal API but not this API, to be added
	// check comp_id is not an ensemble set, to be added

	EXECUTION_REPORT(REPORT_ERROR, member_comp_id, num_comps > 0, "ERROR happens when calling the API \"CCPL_register_ensemble_set_of_components\" at the model code with the annotation \"%s\": the number of component models (corresponding to the input parameter of \"num_comps\") in the new ensemble set is zero (must be larger than 0). Please verify.", annotation);
	check_API_parameter_int(member_comp_id, API_ID_COMP_MGT_REG_ENSEMBLE_SET, member_comp_node->get_comm_group(), "the number of component models", num_comps, "num_comps", annotation);
	for (int i = 0; i < num_comps; i ++) {
		check_API_parameter_string(member_comp_id, API_ID_COMP_MGT_REG_ENSEMBLE_SET, member_comp_node->get_comm_group(), "names of component models", temp_comps_name[i], "comps_names", annotation);
		for (int j = i + 1; j < num_comps; j ++)
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, member_comp_id, !words_are_the_same(temp_comps_name[i], temp_comps_name[j]), "ERROR happens when calling the API \"CCPL_register_ensemble_set_of_components\" at the model code with the annotation \"%s\": the input parameter of \"comps_names\" specifies the same common name of component models (\"%s\"), which is not allowed. Please verify.", temp_comps_name[i]);
	}	

	EXECUTION_REPORT(REPORT_ERROR, member_comp_id, ensemble_size > 0, "ERROR happens when calling the API \"CCPL_register_ensemble_set_of_components\" at the model code with the annotation \"%s\": the input parameter of \"ensemble_size\" (currently is %d) must be larger than 0. Please verify.", annotation, ensemble_size);
	check_API_parameter_int(member_comp_id, API_ID_COMP_MGT_REG_ENSEMBLE_SET, member_comp_node->get_comm_group(), "the number of members in the ensemble", num_comps, "ensemble_size", annotation);
	
	for (int i = 0; i < num_comps; i ++) {
		EXECUTION_REPORT_LOG(REPORT_LOG, member_comp_id, true, "Try to load the information of %d members in the ensemble of the component model \"%s\". Dead lock may happen at this step when the common component name of the ensemble is wrong.", ensemble_size, temp_comps_name[i]);
		while(1) {
			comp_comm_group_mgt_mgr->load_comp_info_of_ensemble(temp_comp_nodes, member_comp_id, temp_comps_name[i], ensemble_size, member_comp_node->get_comm_group(), member_comp_node->get_current_proc_local_id());			
			if (temp_comp_nodes.size() == ensemble_size)
				break;
			EXECUTION_REPORT(REPORT_ERROR, member_comp_id, temp_comp_nodes.size() < ensemble_size, "ERROR happens when calling the API \"CCPL_register_ensemble_set_of_components\" at the model code with the annotation \"%s\": the given \"ensemble_size\" (%d) is smaller than the number (%d) of component models with the common name \"%s\". Please verify.", annotation, ensemble_size, temp_comp_nodes.size(), temp_comps_name[i]);
			for (int j = 0; j < temp_comp_nodes.size(); j ++)
				delete temp_comp_nodes[j];
			temp_comp_nodes.clear();
		}
		for (int j = 0; j < temp_comp_nodes.size(); j ++)
			all_comp_nodes.push_back(temp_comp_nodes[j]);
		temp_comp_nodes.clear();
	}

	count = 0;
	for (int i = 0; i < all_comp_nodes.size(); i ++)
		if (words_are_the_same(all_comp_nodes[i]->get_full_name(), member_comp_node->get_full_name()))
			count ++;
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, count <= 1, "Software error in Comp_comm_group_mgt_mgr::register_ensemble_set_of_comps");
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, count == 1, "ERROR happens when calling the API \"CCPL_register_ensemble_set_of_components\" at the model code with the annotation \"%s\": the current member of the current component model does not belong to the ensemble set to be generated. Please verify", annotation);

	num_total_global_procs = 0;
	for (int i = 0; i < all_comp_nodes.size(); i ++)
		num_total_global_procs += all_comp_nodes[i]->get_num_procs();
	all_procs_global_ids = new int [num_total_global_procs];
	num_total_global_procs = 0;
	for (int i = 0; i < all_comp_nodes.size(); i ++)
		for (int j = 0; j < all_comp_nodes[i]->get_num_procs(); j ++) 
			all_procs_global_ids[num_total_global_procs++] = all_comp_nodes[i]->get_local_proc_global_id(j);
	do_quick_sort(all_procs_global_ids, (int*)NULL, 0, num_total_global_procs-1);

	for (int i = 1; i < num_total_global_procs; i ++) 
		EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, all_procs_global_ids[i-1] != all_procs_global_ids[i], "Software error in Comp_comm_group_mgt_mgr::register_ensemble_set_of_comps");

	if (all_comp_nodes.size() == 1)
		new_set_comm = member_comp_node->get_comm_group(); 
	else {
		int *members_root_proc_ids = new int [all_comp_nodes.size()];
		int *ensemble_member_index_map = new int [all_comp_nodes.size()];
		std::vector < int > merged_procs_global_ids, current_procs_global_ids;
		std::vector < Comp_comm_group_mgt_node * > sorted_all_comp_nodes;
		int local_comp_index = -1, current_proc_merged;
		MPI_Comm set_comm_before_merge, set_comm_after_merge = MPI_COMM_NULL, current_comp_comm_group;
		for (int i = 0; i < all_comp_nodes.size(); i ++) {
			ensemble_member_index_map[i] = i;
			members_root_proc_ids[i] = all_comp_nodes[i]->get_root_proc_global_id();
		}
		do_quick_sort(members_root_proc_ids, ensemble_member_index_map, 0, all_comp_nodes.size()-1);
		for (int i = 0; i < all_comp_nodes.size(); i ++) {
			sorted_all_comp_nodes.push_back(all_comp_nodes[ensemble_member_index_map[i]]);
			EXECUTION_REPORT(REPORT_ERROR, -1, sorted_all_comp_nodes[i]->get_root_proc_global_id() == members_root_proc_ids[i], "Software error in Comp_comm_group_mgt_mgr::register_ensemble_set_of_comps");
			if (member_comp_node->get_root_proc_global_id() == members_root_proc_ids[i]) {
				EXECUTION_REPORT(REPORT_ERROR, -1, local_comp_index == -1, "Software error in Comp_comm_group_mgt_mgr::register_ensemble_set_of_comps");
				local_comp_index = i;
			}
		}
		EXECUTION_REPORT(REPORT_ERROR, -1, local_comp_index != -1, "Software error in Comp_comm_group_mgt_mgr::register_ensemble_set_of_comps");
		merged_procs_global_ids.clear();
		for (int i = 0; i < sorted_all_comp_nodes.size(); i ++) {
			if (i >= local_comp_index) {
				set_comm_before_merge = set_comm_after_merge;
				current_comp_comm_group = MPI_COMM_NULL;
				current_proc_merged = 1;
				if (i == local_comp_index) {
					current_comp_comm_group = member_comp_node->get_comm_group();
					current_proc_merged = -1;
				}
				if (i == 0) {				
					EXECUTION_REPORT(REPORT_ERROR, -1, local_comp_index == 0, "Software error in Comp_comm_group_mgt_mgr::register_ensemble_set_of_comps");
					set_comm_after_merge = current_comp_comm_group;
				}
				else {
					current_procs_global_ids.clear();
					for (int j = 0; j < sorted_all_comp_nodes[i]->get_num_procs(); j ++)
						current_procs_global_ids.push_back(sorted_all_comp_nodes[i]->get_local_proc_global_id(j));
					EXECUTION_REPORT(REPORT_ERROR, -1, current_comp_comm_group == MPI_COMM_NULL && set_comm_before_merge != MPI_COMM_NULL || current_comp_comm_group != MPI_COMM_NULL && set_comm_before_merge == MPI_COMM_NULL, "Software error in Comp_comm_group_mgt_mgr::register_ensemble_set_of_comps");
					set_comm_after_merge = create_union_comm_common(set_comm_before_merge, current_comp_comm_group, current_proc_merged, -current_proc_merged, merged_procs_global_ids, current_procs_global_ids, i, NULL, NULL);
				}
			}
			for (int j = 0; j < sorted_all_comp_nodes[i]->get_num_procs(); j ++)
				merged_procs_global_ids.push_back(sorted_all_comp_nodes[i]->get_local_proc_global_id(j));
		}
		new_set_comm = set_comm_after_merge;
		delete [] members_root_proc_ids;
		delete [] ensemble_member_index_map;
	}
	
	delete [] all_procs_global_ids;
	for (int i = 0; i < all_comp_nodes.size(); i ++)
		delete all_comp_nodes[i];

	int new_set_comp_id = register_component(comps_ensemble_set_name, COMP_TYPE_ACTIVE_COUPLED, new_set_comm, global_node_array[0]->get_comp_id(), false, 0, annotation);
	Comp_comm_group_mgt_node *new_set_comp_node = search_global_node(new_set_comp_id);
	new_set_comp_node->add_child(member_comp_node);
	
	return new_set_comp_id;
}
...
int Comp_comm_group_mgt_mgr::register_component(const char *comp_name, const char *comp_type, MPI_Comm &comm, int parent_local_id, bool enabled_in_parent_coupling_gen, int change_dir, const char *annotation)
{
    ...
    if (parent_local_id == -1) {
		...
		external_procedures_mgr->add_model_external_procedure_fields_names_mappers(-1);
	...
	if (new_comp->is_real_component_model())
		external_procedures_mgr->add_model_external_procedure_fields_names_mappers(new_comp->get_local_node_id());
...
void Comp_comm_group_mgt_mgr::load_comp_info_of_ensemble(std::vector<Comp_comm_group_mgt_node *> &comp_nodes, int host_comp_id, const char *ensemble_comp_common_full_name, int ensemble_size, MPI_Comm comm, int current_proc_local_id)
{
    char individual_comp_full_name[NAME_STR_SIZE], XML_file_common_name[NAME_STR_SIZE];
    TiXmlDocument *XML_file;
    int i, glob_return, file_number;
	glob_t pglob;


    sprintf(XML_file_common_name, "%s/%s.basic_info.xml", comp_comm_group_mgt_mgr->get_components_processes_dir(), ensemble_comp_common_full_name);

	if (comm == MPI_COMM_NULL || current_proc_local_id == 0) {
		glob_return = glob(XML_file_common_name, 0, NULL, &pglob);
		EXECUTION_REPORT(REPORT_ERROR, -1, glob_return != GLOB_NOSPACE && glob_return != GLOB_ABORTED, "Software error of glob in Comp_comm_group_mgt_mgr::load_comp_info_of_ensemble");
		file_number = pglob.gl_pathc;
		if (file_number < ensemble_size)
			globfree(&pglob);
	}
	if (comm != MPI_COMM_NULL)
		EXECUTION_REPORT(REPORT_ERROR,-1, MPI_Bcast(&file_number, 1, MPI_INT, 0, comm) == MPI_SUCCESS);
	if (file_number < ensemble_size)
		return;
	
	for (int i = 0; i < file_number; i ++) {
		if (comm == MPI_COMM_NULL || current_proc_local_id == 0) {
			strncpy(individual_comp_full_name, pglob.gl_pathv[i]+strlen(comp_comm_group_mgt_mgr->get_components_processes_dir())+1, strlen(pglob.gl_pathv[i]+strlen(comp_comm_group_mgt_mgr->get_components_processes_dir())+1)-strlen(".basic_info.xml"));
			individual_comp_full_name[strlen(pglob.gl_pathv[i]+strlen(comp_comm_group_mgt_mgr->get_components_processes_dir())+1)-strlen(".basic_info.xml")] = '\0';
		}
		if (comm != MPI_COMM_NULL)
			EXECUTION_REPORT(REPORT_ERROR,-1, MPI_Bcast(individual_comp_full_name, NAME_STR_SIZE, MPI_CHAR, 0, comm) == MPI_SUCCESS);
		comp_nodes.push_back(load_comp_info_from_XML(host_comp_id, individual_comp_full_name, comm));
	}
...
int Comp_comm_group_mgt_mgr::get_num_members_in_ensemble_set(Comp_comm_group_mgt_node *set_comp_node, Comp_comm_group_mgt_node *member_comp_node)
{
	char member_comp_name[NAME_STR_SIZE];
	char *all_members_comp_name;
	int num_members_in_ensemble_set, j;
	std::vector<const char *> distinct_comp_names;


	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, set_comp_node->is_given_comp_a_child(member_comp_node), "Software error in Comp_comm_group_mgt_mgr::get_num_members_in_ensemble_set");
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, strlen(member_comp_node->get_comp_name()) < NAME_STR_SIZE, "Software error in Comp_comm_group_mgt_mgr::get_num_members_in_ensemble_set");
	if (set_comp_node->get_num_members_in_ensemble() != -1)
		return set_comp_node->get_num_members_in_ensemble();
	
	strcpy(member_comp_name, member_comp_node->get_full_name());
	if (set_comp_node->get_current_proc_local_id() == 0)
		all_members_comp_name = new char [set_comp_node->get_num_procs()*NAME_STR_SIZE];
	EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Gather(member_comp_name, NAME_STR_SIZE, MPI_CHAR, all_members_comp_name, NAME_STR_SIZE, MPI_CHAR, 0, set_comp_node->get_comm_group()) == MPI_SUCCESS);
	if (set_comp_node->get_current_proc_local_id() == 0) {
		for (int i = 0; i < set_comp_node->get_num_procs(); i ++) {
			for (j = 0; j < distinct_comp_names.size(); j ++)
				if (words_are_the_same(all_members_comp_name+NAME_STR_SIZE*i, distinct_comp_names[j]))
					break;
			if (j == distinct_comp_names.size())
				distinct_comp_names.push_back(all_members_comp_name+NAME_STR_SIZE*i);
		}
		delete all_members_comp_name;
		num_members_in_ensemble_set = distinct_comp_names.size();
	}
	EXECUTION_REPORT(REPORT_ERROR,-1, MPI_Bcast(&num_members_in_ensemble_set, 1, MPI_INT, 0, set_comp_node->get_comm_group()) == MPI_SUCCESS);
	set_comp_node->set_num_members_in_ensemble(num_members_in_ensemble_set);
	
	return num_members_in_ensemble_set;
}
