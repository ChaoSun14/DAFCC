...
bool are_two_coord_arrays_same(double *array1, double *array2, int array_size1, int array_size2, bool exactly)
{
    double eps = 0.00001;
    ...
    for (int i = 0; i < array_size1; i ++)
        ...
        else if (fabs(array1[i]-array2[i]) >= eps)
            return false;
...
bool are_data_buffers_overlapping(void *data_buf1, int size1, const char *data_type1, void *data_buf2, int size2, const char *data_type2)
{
	if (size1 == 0 || size2 == 0 || data_buf1 == NULL || data_buf2 == NULL)
		return false;
	
	long buf1_start = (long)data_buf1;
	long buf1_end = buf1_start + size1*get_data_type_size(data_type1);
	long buf2_start = (long)data_buf2;
	long buf2_end = buf2_start + size2*get_data_type_size(data_type2);

	return buf2_start >= buf1_start && buf2_end > buf1_start || buf1_start >= buf2_start && buf1_end > buf2_start;
}
