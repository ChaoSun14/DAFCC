...
Connection_coupling_procedure::Connection_coupling_procedure(Inout_interface *inout_interface, Coupling_connection *coupling_connection)
{
    ...
    for (int i = 0; i < coupling_connection->src_fields_info.size(); i ++) {
        ...
        if (inout_interface->get_interface_type() == COUPLING_INTERFACE_MARK_EXPORT && !(inout_interface->get_parent_interface() != NULL && inout_interface->get_parent_interface()->get_interface_type() == COUPLING_INTERFACE_MARK_HALO_EXCHANGE)) {
            if (fields_time_info_dst->inst_or_aver == USING_AVERAGE_VALUE)
                fields_mem_inner_step_averaged[i] = memory_manager->alloc_mem(fields_mem_registered[i], BUF_MARK_AVERAGED_INNER, coupling_connection->connection_id, NULL, false);
            ...
            if (fields_time_info_dst->inst_or_aver == USING_AVERAGE_VALUE && !(fields_time_info_dst->lag_seconds == 0 && fields_time_info_dst->timer->is_the_same_with(fields_time_info_src->timer)))
                fields_mem_inter_step_averaged[i] = memory_manager->alloc_mem(fields_mem_registered[i], BUF_MARK_AVERAGED_INTER, coupling_connection->connection_id, NULL, false);
        ...
        if (inout_interface->get_interface_type() == COUPLING_INTERFACE_MARK_IMPORT && !(inout_interface->get_parent_interface() != NULL && inout_interface->get_parent_interface()->get_interface_type() == COUPLING_INTERFACE_MARK_HALO_EXCHANGE)) {
			if (coupling_connection->dst_fields_info[i]->runtime_remapping_weights_container == NULL)
				EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, coupling_connection->dst_fields_info[i]->runtime_remapping_weights_under_V3D == NULL, "software error in Connection_coupling_procedure::Connection_coupling_procedure");
            if (coupling_connection->dst_fields_info[i]->runtime_remapping_weights_container == NULL || coupling_connection->dst_fields_info[i]->runtime_remapping_weights_container->is_empty()) {
				Original_grid_info *src_original_grid = NULL, *dst_original_grid = NULL;
				if (coupling_connection->dst_fields_info[i]->runtime_remapping_weights_container == NULL && words_are_the_same(coupling_connection->get_src_comp_full_name(), coupling_connection->get_dst_comp_full_name()) && !words_are_the_same(coupling_connection->dst_fields_info[i]->grid_name, coupling_connection->src_fields_info[i]->grid_name)) {
					src_original_grid = original_grid_mgr->search_grid_info(coupling_connection->src_fields_info[i]->grid_name, inout_interface->get_comp_id());
					dst_original_grid = original_grid_mgr->search_grid_info(coupling_connection->dst_fields_info[i]->grid_name, inout_interface->get_comp_id());
					EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, src_original_grid != NULL && (src_original_grid->get_original_CoR_grid()->is_similar_grid_with(dst_original_grid->get_original_CoR_grid()) || src_original_grid->is_the_same_as_another_grid(dst_original_grid)), "Software error");
					if (!src_original_grid->get_original_CoR_grid()->is_similar_grid_with(dst_original_grid->get_original_CoR_grid()))
						src_original_grid = NULL;
				}
				if (src_original_grid != NULL) {
					EXECUTION_REPORT_LOG(REPORT_LOG, inout_interface->get_comp_id(), true, "Detect similar grids for generating data transfer");
					fields_mem_transfer[i] = memory_manager->alloc_mem(fields_mem_registered[i]->get_field_name(), fields_mem_registered[i]->get_decomp_id(), 
    	                                                               src_original_grid->get_grid_id(), BUF_MARK_DATA_TRANSFER^coupling_connection->connection_id, 
				...
				else fields_mem_transfer[i] = memory_manager->alloc_mem(fields_mem_registered[i], BUF_MARK_DATA_TRANSFER, coupling_connection->connection_id, transfer_data_type, false);
            }
            else {
				if (coupling_connection->dst_fields_info[i]->runtime_remapping_weights_under_V3D != NULL) 
	                fields_mem_transfer[i] = memory_manager->alloc_mem(fields_mem_registered[i]->get_field_name(), coupling_connection->dst_fields_info[i]->runtime_remapping_weights_under_V3D->get_src_decomp_info()->get_decomp_id(), 
    	                                                               coupling_connection->dst_fields_info[i]->runtime_remapping_weights_container->get_src_original_grid()->get_grid_id(), BUF_MARK_DATA_TRANSFER^coupling_connection->connection_id, transfer_data_type, fields_mem_registered[i]->get_unit(), "internal", false);
				else fields_mem_transfer[i] = memory_manager->alloc_mem(fields_mem_registered[i]->get_field_name(), fields_mem_registered[i]->get_decomp_id(), 
    	                                                                coupling_connection->dst_fields_info[i]->runtime_remapping_weights_container->get_src_original_grid()->get_grid_id(), BUF_MARK_DATA_TRANSFER^coupling_connection->connection_id, 
        	                                                            transfer_data_type, fields_mem_registered[i]->get_unit(), "internal", inout_interface->get_interface_source() == INTERFACE_SOURCE_REGISTER && i < coupling_connection->fields_name.size());
                fields_mem_remapped[i] = memory_manager->alloc_mem(fields_mem_registered[i]->get_field_name(), fields_mem_registered[i]->get_decomp_id(), fields_mem_registered[i]->get_grid_id(), 
                                                                   BUF_MARK_REMAP_NORMAL^coupling_connection->connection_id, transfer_data_type, fields_mem_registered[i]->get_unit(), "internal", false);
                runtime_remap_algorithms[i] = new Runtime_remap_algorithm(coupling_connection->dst_fields_info[i]->runtime_remapping_weights_container, fields_mem_transfer[i], fields_mem_remapped[i], coupling_connection->connection_id);
            }
            if (!words_are_the_same(transfer_data_type, coupling_connection->dst_fields_info[i]->data_type)) {
                ...
                fields_mem_datatype_transformed[i] = memory_manager->alloc_mem(fields_mem_registered[i], BUF_MARK_DATATYPE_TRANS, coupling_connection->connection_id, coupling_connection->dst_fields_info[i]->data_type, false);
        ...
        if (inout_interface->get_interface_type() == COUPLING_INTERFACE_MARK_EXPORT) {
            ...
            else if (fields_mem_inter_step_averaged[i] != NULL) 
				fields_mem_transfer[i] = fields_mem_inter_step_averaged[i];
			...
			if (inout_interface->get_parent_interface() != NULL && inout_interface->get_parent_interface()->get_interface_type() == COUPLING_INTERFACE_MARK_HALO_EXCHANGE)
				fields_mem_transfer[i] = fields_mem_registered[i];
            else runtime_inter_averaging_algorithm[i] = new Runtime_cumulate_average_algorithm(this, last_field_instance, fields_mem_registered[i]);
        }
    }
...
void Connection_coupling_procedure::execute(bool bypass_timer, int *field_update_status, const char *annotation)
{
	...
	if (inout_interface->get_parent_interface() != NULL && inout_interface->get_parent_interface()->get_interface_type() == COUPLING_INTERFACE_MARK_HALO_EXCHANGE)
		EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, false, "Software error in Connection_coupling_procedure::execute");
...
void Connection_coupling_procedure::do_halo_exchange()
{
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, runtime_data_transfer_algorithm != NULL, "Software error in Connection_coupling_procedure::do_halo_exchange");
	
	current_remote_fields_time = -1;
	runtime_data_transfer_algorithm->set_for_halo_exchange();
	runtime_data_transfer_algorithm->pass_transfer_parameters(current_remote_fields_time, inout_interface->get_bypass_counter());
    ...
    for (i = 0; i < num_total_fields; i ++) {
        ...
        for (j = 0; j < fields_mem_registered.size(); j ++)
            if (words_are_the_same(restart_field_name, fields_mem_registered[j]->get_field_name()))
                break;
		EXECUTION_REPORT(REPORT_ERROR, inout_interface->get_comp_id(), fields_mem_registered.size() > i && words_are_the_same(restart_field_name, fields_mem_registered[i]->get_field_name()), "Error happens when loading the restart data file \"%s\": it does not match the configuration of the interface \"%s\": its original %dth field recorded in the restart data file is \"%s\" while the current %dth field is \"%s\". Please check.", restart_buffer->get_input_restart_mgt_info_file(), inout_interface->get_interface_name(), i+1, restart_field_name, j+1, fields_mem_registered[i]->get_field_name());
    }
...
Inout_interface::Inout_interface(const char *interface_name, int interface_id, int num_fields, int *field_ids_src, int *field_ids_dst, int timer_id, int inst_or_aver, int array_size_src, int array_size_dst, int API_id, const char *annotation)
{
    char child_interface_name[NAME_STR_SIZE], API_label[NAME_STR_SIZE];
	int local_API_id = API_id;


	get_API_hint(-1, API_id, API_label);
	
    sprintf(child_interface_name, "%s_child_export", interface_name);
    children_interfaces.push_back(new Inout_interface(child_interface_name, -1, 1, num_fields, field_ids_src, array_size_src, timer_id, inst_or_aver, "field_instance_IDs_source", annotation, API_id, INTERFACE_SOURCE_REGISTER, true));
    sprintf(child_interface_name, "%s_child_import", interface_name);
    children_interfaces.push_back(new Inout_interface(child_interface_name, -1, 0, num_fields, field_ids_dst, array_size_dst, timer_id, inst_or_aver, "field_instance_IDs_target", annotation, API_id, INTERFACE_SOURCE_REGISTER, true));
	...
	children_interfaces[0]->parent_interface = this;
	children_interfaces[1]->parent_interface = this;
    ...
    for (int i = 0; i < num_fields; i ++) {    
		...
		if (memory_manager->get_field_instance(field_ids_src[i])->get_data_buf() != NULL)		
...
Inout_interface::Inout_interface(const char *interface_name, int interface_id, int num_field_instances, int *field_instance_IDs, int *halo_region_IDs, int size_field_instance_IDs, int size_halo_region_IDs, const char *annotation)
{
    char child_interface_name[NAME_STR_SIZE];
    int *field_instance_IDs_halo, default_timer_id;
    Field_mem_info *field_instance_halo;
    

    common_checking_for_interface_registration(num_field_instances, field_instance_IDs, size_field_instance_IDs, -1, USING_INSTANTANEOUS_VALUE, 4, interface_name, API_ID_INTERFACE_REG_HALO_EXCHANGE, INTERFACE_SOURCE_REGISTER, "field_instance_IDs", annotation);
    for (int i = 0; i < num_field_instances; i ++)
        fields_mem_registered.push_back(memory_manager->get_field_instance(field_instance_IDs[i]));
    default_timer_id = timer_mgr->define_pseudo_timer(fields_mem_registered[0]->get_comp_id());
    initialize_data(interface_name, interface_id, 4, default_timer_id, USING_INSTANTANEOUS_VALUE, field_instance_IDs, INTERFACE_SOURCE_REGISTER, annotation);
    EXECUTION_REPORT(REPORT_ERROR, comp_id, num_field_instances <= size_halo_region_IDs, "Error happens when calling API \"CCPL_register_halo_exchange_interface\" to register an interface named \"%s\": the array size of parameter \"halo_region_IDs\" cannot be smaller than the parameter \"num_field_instances\". Please verify the model code with the annotation \"%s\".", interface_name, annotation);
    for (int i = 0; i < num_field_instances; i ++) {
        EXECUTION_REPORT(REPORT_ERROR, comp_id, decomps_info_mgr->is_decomp_id_legal(halo_region_IDs[i]), "Error happens when calling API \"CCPL_register_halo_exchange_interface\" to register an interface named \"%s\": the No. %d value of parameter \"halo_region_IDs\" is not a legal halo region ID. Please verify the model code with the annotation \"%s\".", interface_name, i+1, annotation);
        Decomp_info *halo_decomp = decomps_info_mgr->get_decomp_info(halo_region_IDs[i]);
        EXECUTION_REPORT(REPORT_ERROR, comp_id, comp_id == halo_decomp->get_comp_id(), "Error happens when calling API \"CCPL_register_halo_exchange_interface\" to register an interface named \"%s\": the halo region corresponding to the No. %d value of parameter \"halo_region_IDs\" does not belong to the same component model with the coupling interface. Please verify the model code with the annotation \"%s\".", interface_name, i+1, annotation);
        EXECUTION_REPORT(REPORT_ERROR, comp_id, halo_decomp->get_halo_host_decomp() != NULL, "Error happens when calling API \"CCPL_register_halo_exchange_interface\" to register an interface named \"%s\": the No. %d element (named \"%s\") of parameter \"halo_region_IDs\" is not a legal halo region. Please verify the model code with the annotation \"%s\".", interface_name, i+1, halo_decomp->get_decomp_name(), annotation);
        EXECUTION_REPORT(REPORT_ERROR, comp_id, fields_mem_registered[i]->get_grid_id() != -1, "Error happens when calling API \"CCPL_register_halo_exchange_interface\" to register an interface named \"%s\": the grid of No. %d field instance does not contain a horizontal sub grid. Please verify the model code with the annotation \"%s\".", interface_name, i+1, annotation);
        Original_grid_info *field_grid = original_grid_mgr->get_original_grid(fields_mem_registered[i]->get_grid_id());
        EXECUTION_REPORT(REPORT_ERROR, comp_id, field_grid->get_H2D_sub_CoR_grid() != NULL, "Error happens when calling API \"CCPL_register_halo_exchange_interface\" to register an interface named \"%s\": the grid (named \"%s\") of No. %d field instance does not contain a horizontal sub grid. Please verify the model code with the annotation \"%s\".", interface_name, field_grid->get_grid_name(), i+1, annotation);
        Original_grid_info *halo_grid = original_grid_mgr->get_original_grid(halo_decomp->get_grid_id());
        EXECUTION_REPORT(REPORT_ERROR, comp_id, fields_mem_registered[i]->get_decomp_id() == halo_decomp->get_halo_host_decomp()->get_decomp_id(), "Error happens when calling API \"CCPL_register_halo_exchange_interface\" to register an interface named \"%s\": the No. %d field instance (\"%s\") and halo region (\"%s\") do not correspond to the same parallel decomposition. Please verify the model code with the annotation \"%s\".", interface_name, i+1, fields_mem_registered[i]->get_field_name(), halo_decomp->get_decomp_name(), annotation);
        check_API_parameter_string(comp_id, API_ID_INTERFACE_REG_HALO_EXCHANGE, comp_comm_group_mgt_mgr->get_comm_group_of_local_comp(comp_id, "Inout_interface for halo"), "registering a halo exchange interface", halo_decomp->get_decomp_name(), "\"halo_region_IDs\"", annotation);
        halo_regions.push_back(halo_decomp);
    }
    
    sprintf(child_interface_name, "%s_child_export", interface_name);
    children_interfaces.push_back(new Inout_interface(child_interface_name, -1, 1, num_field_instances, field_instance_IDs, num_field_instances, default_timer_id, USING_AVERAGE_VALUE, "field_instance_IDs", annotation, API_ID_INTERFACE_REG_HALO_EXCHANGE, INTERFACE_SOURCE_REGISTER, true));

    field_instance_IDs_halo = new int [num_field_instances];
    for (int i = 0; i < num_field_instances; i ++) {
        field_instance_halo = memory_manager->alloc_mem(fields_mem_registered[i]->get_field_name(), halo_region_IDs[i], fields_mem_registered[i]->get_grid_id(), BUF_MARK_HALO_EXCHANGE, fields_mem_registered[i]->get_data_type(), fields_mem_registered[i]->get_unit(), "C-Coupler internal", false);
        field_instance_halo->reset_mem_buf(fields_mem_registered[i], REG_FIELD_TAG_NONE);
        field_instance_IDs_halo[i] = field_instance_halo->get_field_instance_id();
    }
    sprintf(child_interface_name, "%s_child_import", interface_name);
    children_interfaces.push_back(new Inout_interface(child_interface_name, -1, 0, num_field_instances, field_instance_IDs_halo, num_field_instances, default_timer_id, USING_AVERAGE_VALUE, "field_instance_IDs", annotation, API_ID_INTERFACE_REG_HALO_EXCHANGE, INTERFACE_SOURCE_REGISTER, true));    
    delete [] field_instance_IDs_halo;
    fields_mem_registered.clear();
	children_interfaces[0]->parent_interface = this;
	children_interfaces[1]->parent_interface = this;
}
...
void Inout_interface::initialize_data(const char *interface_name, int interface_id, int interface_type, int timer_id, int inst_or_aver, int *field_ids, int interface_source, const char *annotation)
{
    ...
    this->fields_name.clear();
    this->is_last_halo_exchange_waited = true;
...
{
    EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "finalize inout interface %s : %lx", interface_name, this);
    if (children_interfaces.size() > 0)
        EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "will finalize inout interface child %s : %lx", children_interfaces[0]->get_interface_name(), children_interfaces[0]);
    ...
    for (int i = 0; i < fields_name.size(); i ++) {        
        EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "inout interface %s %lx release %lx", interface_name, this, fields_name[i]);
...
void Inout_interface::common_checking_for_interface_registration(int num_fields, int *field_ids, int array_size, int timer_id, int inst_or_aver, int interface_type, const char *interface_name, int API_id, int interface_source, const char *field_ids_parameter_name, const char *annotation)
{
    ...
    for (int i = 0; i < num_fields; i ++) {
        if (interface_source != INTERFACE_SOURCE_IO_WRITE)
            EXECUTION_REPORT(REPORT_ERROR, -1, memory_manager->check_is_legal_field_instance_id(field_ids[i]) && (memory_manager->get_field_instance(field_ids[i])->get_is_registered_model_buf() || API_id == API_ID_EXTERNAL_PROC_INST_INIT), "Error happens when calling the API \"%s\" to register an interface named \"%s\": the parameter \"%s\" contains wrong field instance ID (the No. %d element of the array is wrong or the registration for the corresponding chunk field instance has not been finalized). Please verify the model code related to the annotation \"%s\"", API_label, interface_name, field_ids_parameter_name, i+1, annotation);
        if (i == 0)
            comp_id = memory_manager->get_field_instance(field_ids[i])->get_comp_id();
        EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, comp_id, comp_id == memory_manager->get_field_instance(field_ids[i])->get_comp_id(), "Error happens when calling the API \"%s\" to register an interface named \"%s\": the field instances specified via the parameter \"%s\" should but not correspond to the same component model currently: the first field instance corresponds to the component model \"%s\" while the No. %d field instance corresponds to the component model \"%s\". Please verify the model code with the annotation \"%s\".", API_label, interface_name, field_ids_parameter_name, comp_comm_group_mgt_mgr->get_global_node_of_local_comp(comp_id,false, "")->get_comp_full_name(), i+1, comp_comm_group_mgt_mgr->get_global_node_of_local_comp(memory_manager->get_field_instance(field_ids[i])->get_comp_id(),false, "")->get_comp_full_name(), annotation);
    }
    if (interface_type != COUPLING_INTERFACE_MARK_HALO_EXCHANGE && API_id != API_ID_EXTERNAL_PROC_INST_INIT) {
	...
	if (API_id != API_ID_EXTERNAL_PROC_INST_INIT)
...
void Inout_interface::execute(bool bypass_timer, int API_id, int *field_update_status, int size_field_update_status, const char *annotation)
{
    ...
    if (interface_type == COUPLING_INTERFACE_MARK_HALO_EXCHANGE) {
        char API_label[NAME_STR_SIZE];
        get_API_hint(comp_id, API_id, API_label);
        EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, comp_id, false, "Error happens when calling API \"%s\" to execute the halo exchange interface \"%s\": please call API \"CCPL_execute_halo_exchange_using_id\" or \"CCPL_execute_halo_exchange_using_name\" instead. Please check the model code with the annotation \"%s\"", API_label, interface_name, annotation);
    }

	if (parent_interface != NULL && parent_interface->get_interface_type() == COUPLING_INTERFACE_MARK_HALO_EXCHANGE) {
		EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, coupling_procedures.size() == 1, "Software error in Inout_interface::execute");
		bypass_counter ++;
		coupling_procedures[0]->do_halo_exchange();
		return;
	}
    ...
    if (interface_type == COUPLING_INTERFACE_MARK_NORMAL_REMAP || interface_type == COUPLING_INTERFACE_MARK_FRAC_REMAP) {
        for (int i = 0; i < fields_mem_registered.size(); i ++)
            fields_mem_registered[i]->check_field_sum(report_internal_log_enabled, true, "before executing a remap interface");
        if (interface_type == COUPLING_INTERFACE_MARK_FRAC_REMAP)
    ...
    if (interface_type == COUPLING_INTERFACE_MARK_EXPORT) {
        for (int i = 0; i < fields_mem_registered.size(); i ++)
            fields_mem_registered[i]->check_field_sum(report_internal_log_enabled, true, "before executing an export interface");
    ...
    if (interface_type == COUPLING_INTERFACE_MARK_IMPORT) {
        for (int i = 0; i < fields_mem_registered.size(); i ++)
            fields_mem_registered[i]->check_field_sum(report_internal_log_enabled, true, "after executing an import interface");
    }
}


void Inout_interface::do_halo_exchange(int API_id, bool is_asynchronous, const char *annotation)
{
    int field_update_status[4096];
    char API_label[NAME_STR_SIZE];


	for (int i = 0; i < children_interfaces[0]->fields_mem_registered.size(); i ++)
		children_interfaces[0]->fields_mem_registered[i]->check_field_sum(report_internal_log_enabled, true, "before halo exchanging");

    if (interface_type != COUPLING_INTERFACE_MARK_HALO_EXCHANGE) {
        get_API_hint(comp_id, API_id, API_label);
        EXECUTION_REPORT(REPORT_ERROR, comp_id, false, "Error happens when calling API \"%s\" to execute the coupling interface \"%s\": this API cannot be used because the coupling interface is not a halo exchange interface. Please check the model code with the annotation \"%s\"", API_label, interface_name, annotation);
    }
    if (!is_last_halo_exchange_waited) {
        get_API_hint(comp_id, API_id, API_label);
        EXECUTION_REPORT(REPORT_ERROR, comp_id, false, "Error happens when calling API \"%s\" to execute the halo exchange interface \"%s\": the last call for executing this interface is an asynchorous halo exchange while the model does not confirm that the asynchorous halo exchange has been ended safely. Please check the model code with the annotation \"%s\"", API_label, interface_name, annotation);
    }
    children_interfaces[0]->execute(true, API_id, field_update_status, 4096, annotation);
    if (!is_asynchronous)
        children_interfaces[1]->execute(true, API_id, field_update_status, 4096, annotation);
    is_last_halo_exchange_waited = (!is_asynchronous);
}


void Inout_interface::finish_halo_exchange(int API_id, const char *annotation)
{
    int field_update_status[4096];
    char API_label[NAME_STR_SIZE];
    ...
    children_interfaces[1]->execute(true, API_id, field_update_status, 4096, annotation);
    is_last_halo_exchange_waited = true;
	for (int i = 0; i < children_interfaces[0]->fields_mem_registered.size(); i ++)
		children_interfaces[0]->fields_mem_registered[i]->check_field_sum(report_internal_log_enabled, true, "after halo exchanging");
}
...
void Inout_interface::add_remappling_fraction_processing(void *frac_src, void *frac_dst, int size_frac_src, int size_frac_dst, const char *frac_data_type, int API_id, const char *annotation)
{
	char API_label[NAME_STR_SIZE];
    ...
    for (int i = 0; i < children_interfaces[0]->fields_mem_registered.size(); i ++) {
        ...
        children_interfaces[0]->fields_mem_registered[i] = memory_manager->alloc_mem(fields_mem_registered[i], BUF_MARK_REMAP_FRAC, coupling_generator->get_latest_connection_id(), fields_mem_registered[i]->get_data_type(), false);
    }
...
//	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, do_coupling_generation_between_interfaces_intra_one_component_model(child_interface_import,child_interface_export,-1, has_frac_remapping));
...
int Inout_interface_mgt::register_normal_remap_interface(const char *interface_name, int num_fields, int *field_ids_src, int *field_ids_dst, int timer_id, int inst_or_aver, int array_size_src, int array_size_dst, int API_id, const char *annotation)
{
    char API_label[NAME_STR_SIZE];
    get_API_hint(-1, API_id, API_label);

    Inout_interface *new_interface = new Inout_interface(interface_name, get_next_interface_id(), num_fields, field_ids_src, field_ids_dst, timer_id, inst_or_aver, array_size_src, array_size_dst, API_id, annotation);
...
int Inout_interface_mgt::register_frac_based_remap_interface(const char *interface_name, int num_fields, int *field_ids_src, int *field_ids_dst, int timer_id, int inst_or_aver, int array_size_src, int array_size_dst, void *frac_src, void *frac_dst, int size_frac_src, int size_frac_dst, const char *frac_data_type, int API_id, const char *annotation)
{
    int new_remap_interface_id = register_normal_remap_interface(interface_name, num_fields, field_ids_src, field_ids_dst, timer_id, inst_or_aver, array_size_src, array_size_dst, API_id, annotation);
    ...
    new_remap_interface->add_remappling_fraction_processing(frac_src, frac_dst, size_frac_src, size_frac_dst, frac_data_type, API_id, annotation);
...
int Inout_interface_mgt::register_inout_interface(const char *interface_name, int interface_type, int num_fields, int *field_ids, int array_size, int timer_id, int inst_or_aver, const char *annotation, int interface_source)
{
    ...
    for (int i = 0; i < interfaces.size(); i ++) {
        if (interfaces[i] == NULL || new_interface->get_comp_id() != interfaces[i]->get_comp_id())
            continue;
...
int Inout_interface_mgt::register_halo_exchange_interface(const char *interface_name, int num_field_instances, int *field_instance_IDs, int *halo_region_IDs, int size_field_instance_IDs, int size_halo_region_IDs, const char *annotation)
{
    Inout_interface *new_interface = new Inout_interface(interface_name, get_next_interface_id(), num_field_instances, field_instance_IDs, halo_region_IDs, size_field_instance_IDs, size_halo_region_IDs, annotation);
    for (int i = 0; i < interfaces.size(); i ++) {
        if (interfaces[i] == NULL || new_interface->get_comp_id() != interfaces[i]->get_comp_id())
            continue;
        EXECUTION_REPORT(REPORT_ERROR, new_interface->get_comp_id(), !words_are_the_same(interface_name, interfaces[i]->get_interface_name()), 
                         "cannot register the halo exchange interface named \"%s\" at the model code with the annotation \"%s\" because an interface with the same name has already been registered at the model code with the annotation \"%s\"",
                         interface_name, annotation, annotation_mgr->get_annotation(interfaces[i]->get_interface_id(), "registering interface"));
    }
    generate_remapping_interface_connection(new_interface, num_field_instances, field_instance_IDs, false);
...
bool Inout_interface_mgt::is_interface_id_legal(int interface_id)
{
    ...
    return (interface_id&TYPE_ID_SUFFIX_MASK) < interfaces.size() && interfaces[(interface_id&TYPE_ID_SUFFIX_MASK)] != NULL;
}
...
{
    for (int i = 0; i < interfaces.size(); i ++)
        if (interfaces[i] != NULL && words_are_the_same(interfaces[i]->get_comp_full_name(),comp_full_name) && words_are_the_same(interfaces[i]->get_interface_name(), interface_name))
            return interfaces[i];
...
{
    for (int i = 0; i < interfaces.size(); i ++)
        if (interfaces[i] != NULL && interfaces[i]->get_comp_id() == comp_id && words_are_the_same(interfaces[i]->get_interface_name(), interface_name))
            return interfaces[i];
...
void Inout_interface_mgt::get_all_unconnected_inout_interface_fields_info(std::vector<const char*> &all_descendant_real_comp_fullnames, char **temp_array_buffer, long &buffer_content_size, MPI_Comm comm)
{
    ...
    for (int i = 0; i < all_descendant_real_comp_fullnames.size(); i ++) {
        ...
        for (int j = 0; j < interfaces.size(); j ++)
            if (interfaces[j] != NULL && interfaces[j]->get_comp_id() == comp_node->get_comp_id())
...
void Inout_interface_mgt::get_all_import_interfaces_of_a_component(std::vector<Inout_interface*> &import_interfaces, int comp_id)
{
    ...
    for (int i = 0; i < interfaces.size(); i ++)
        if (interfaces[i] != NULL && interfaces[i]->get_comp_id() == comp_id && interfaces[i]->get_interface_type() == COUPLING_INTERFACE_MARK_IMPORT)
...
void Inout_interface_mgt::get_all_import_interfaces_of_a_component(std::vector<Inout_interface*> &import_interfaces, const char *comp_full_name)
{
    ...
    for (int i = 0; i < interfaces.size(); i ++)
        if (interfaces[i] != NULL && words_are_the_same(interfaces[i]->get_comp_full_name(), comp_full_name) && interfaces[i]->get_interface_type() == COUPLING_INTERFACE_MARK_IMPORT)
            import_interfaces.push_back(interfaces[i]);
}
...
void Inout_interface_mgt::execute_halo_exchange(int interface_id, int API_id, bool is_asynchronous, const char *annotation)
{
    if (!is_interface_id_legal(interface_id)) {
        char API_label[NAME_STR_SIZE];
        get_API_hint(-1, API_id, API_label);
        EXECUTION_REPORT(REPORT_ERROR, -1, false, "Error happens when calling API \"%s\" to execute a coupling interface: the parameter \"interface_id\" is wrong. Please check the model code with the annotation \"%s\"", API_label, annotation);
    }    
    Inout_interface *inout_interface = get_interface(interface_id);
    EXECUTION_REPORT_LOG(REPORT_LOG, inout_interface->get_comp_id(), true, "Begin to execute interface \"%s\" (model code annotation is \"%s\")", inout_interface->get_interface_name(), annotation);
    inout_interface->do_halo_exchange(API_id, is_asynchronous, annotation);
...
void Inout_interface_mgt::execute_halo_exchange(int comp_id, int API_id, const char *interface_name, bool is_asynchronous, const char *annotation)
{
    Inout_interface *inout_interface = get_interface(comp_id, interface_name);
    if (inout_interface == NULL) {
        char API_label[NAME_STR_SIZE];
        get_API_hint(comp_id, API_id, API_label);
        EXECUTION_REPORT(REPORT_ERROR, comp_id, false, "Error happens when calling API \"%s\" to execute an interface \"%s\": cannot find a coupling interface according to the parameters \"comp_id\" and \"interface_name\". Please check the model code with the annotation \"%s\"", API_label, interface_name, annotation);
    }
    EXECUTION_REPORT_LOG(REPORT_LOG, inout_interface->get_comp_id(), true, "Begin to execute interface \"%s\" (model code annotation is \"%s\")", inout_interface->get_interface_name(), annotation);
    inout_interface->do_halo_exchange(API_id, is_asynchronous, annotation);
    EXECUTION_REPORT_LOG(REPORT_LOG, inout_interface->get_comp_id(), true, "Finishing executing interface \"%s\" (model code annotation is \"%s\")", inout_interface->get_interface_name(), annotation);
...
void Inout_interface_mgt::finish_halo_exchange(int interface_id, int API_id, const char *annotation)
{
    if (!is_interface_id_legal(interface_id)) {
        char API_label[NAME_STR_SIZE];
        get_API_hint(-1, API_id, API_label);
    ...
    Inout_interface *inout_interface = get_interface(interface_id);
    EXECUTION_REPORT_LOG(REPORT_LOG, inout_interface->get_comp_id(), true, "Begin to finish halo exchange interface \"%s\" (model code annotation is \"%s\")", inout_interface->get_interface_name(), annotation);
    inout_interface->finish_halo_exchange(API_id, annotation);
    EXECUTION_REPORT_LOG(REPORT_LOG, inout_interface->get_comp_id(), true, "Finishing finishing halo exchange interface \"%s\" (model code annotation is \"%s\")", inout_interface->get_interface_name(), annotation);    
}


void Inout_interface_mgt::finish_halo_exchange(int comp_id, int API_id, const char *interface_name, const char *annotation)
{
    Inout_interface *inout_interface = get_interface(comp_id, interface_name);
    if (inout_interface == NULL) {
        char API_label[NAME_STR_SIZE];
...
void Inout_interface_mgt::write_into_restart_buffers(int comp_id)
{
    ...
    for (int i = 0; i < interfaces.size(); i ++)
        if (interfaces[i] != NULL && interfaces[i]->get_comp_id() == comp_id) {
...
void Inout_interface_mgt::write_comp_export_info_into_XML_file(int comp_id)
{
    ...
    if (comp_node->get_current_proc_local_id() == 0) {
        ...
        XML_file->LinkEndChild(root_element);
        for (int i = 0; i < interfaces.size(); i ++)
            if (interfaces[i] != NULL && interfaces[i]->get_interface_type() == COUPLING_INTERFACE_MARK_EXPORT && interfaces[i]->get_comp_id() == comp_id)
...
{
    for (int i = 0; i < interfaces.size(); i ++)
        if (interfaces[i] != NULL && interfaces[i]->get_comp_id() == comp_id && interfaces[i]->get_interface_type() < 2 && interfaces[i]->has_been_executed_with_timer())
            return interfaces[i];
...
{
    for (int i = 0; i < interfaces.size(); i ++)
        if (interfaces[i] != NULL && interfaces[i]->get_comp_id() == comp_id && interfaces[i]->is_in_restart_write_window())
            return true;
...
void Inout_interface_mgt::finalize_inout_interface(int interface_id)
{
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, is_interface_id_legal(interface_id), "Software error in Inout_interface_mgt::finalize_inout_interface");
	delete interfaces[interface_id&TYPE_ID_SUFFIX_MASK];
	interfaces[interface_id&TYPE_ID_SUFFIX_MASK] = NULL;
}


bool Inout_interface_mgt::do_coupling_generation_between_interfaces_intra_one_component_model(Inout_interface *interface1, Inout_interface *interface2, int API_id, bool has_frac_remapping)
{
	Comp_comm_group_mgt_node *comp1_node = NULL, *comp2_node = NULL, *temp_comp2_node;
	char comp2_full_name[NAME_STR_SIZE], interface2_name[NAME_STR_SIZE];
	int comp2_current_process_id = -1, comp2_max_process_id, i, j;
    std::pair<const char*, const char*> src_comp_interface_info;
	Inout_interface *import_interface = NULL;
	std::vector<const char*> interface1_fields_name, interface2_fields_name;
	bool comp2_node_pushed = false;


	if (interface1 == NULL)
		return false;

	comp1_node = comp_comm_group_mgt_mgr->search_global_node(interface1->get_comp_id());
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, comp1_node != NULL && comp1_node->get_current_proc_local_id() >= 0, "Software error in Inout_interface_mgt::do_coupling_generation_between_interfaces_intra_one_component_model");
	if (interface2 != NULL) {
		comp2_node = comp_comm_group_mgt_mgr->search_global_node(interface2->get_comp_id());
		EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, comp2_node != NULL && comp2_node->get_current_proc_local_id() >= 0, "Software error in Inout_interface_mgt::do_coupling_generation_between_interfaces_intra_one_component_model");		
		EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, comp1_node == comp2_node || comp1_node->is_given_comp_a_child(comp2_node), "Software error in Inout_interface_mgt::do_coupling_generation_between_interfaces_intra_one_component_model");
		strcpy(comp2_full_name, comp2_node->get_full_name());
		strcpy(interface2_name, interface2->get_interface_name());
		comp2_current_process_id = comp1_node->get_current_proc_local_id();
	}	
	synchronize_comp_processes_for_API(comp1_node->get_comp_id(), API_id, comp1_node->get_comm_group(), "in Inout_interface_mgt::do_coupling_generation_between_interfaces_intra_one_component_model", "Inout_interface_mgt::do_coupling_generation_between_interfaces_intra_one_component_model");
	MPI_Allreduce(&comp2_current_process_id, &comp2_max_process_id, 1, MPI_INT, MPI_MAX, comp1_node->get_comm_group());
	if (comp2_max_process_id == -1)
		return false;
	EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Bcast(comp2_full_name, NAME_STR_SIZE, MPI_CHAR, comp2_max_process_id, comp1_node->get_comm_group()) == MPI_SUCCESS);
	EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Bcast(interface2_name, NAME_STR_SIZE, MPI_CHAR, comp2_max_process_id, comp1_node->get_comm_group()) == MPI_SUCCESS);
	if (comp2_node != NULL)
        //EXECUTION_REPORT(REPORT_ERROR, -1, false, "comp2_node->get_full_name(): %s;  comp2_full_name: %s", comp2_node->get_full_name(), comp2_full_name);
		EXECUTION_REPORT(REPORT_ERROR, -1, words_are_the_same(comp2_node->get_full_name(), comp2_full_name), "Software error in Inout_interface_mgt::do_coupling_generation_between_interfaces_intra_one_component_model");
	if (comp1_node != comp2_node) {
		temp_comp2_node = comp_comm_group_mgt_mgr->load_comp_info_from_XML(comp1_node->get_comp_id(), comp2_full_name, comp1_node->get_comm_group());
		if (comp2_node != NULL) {
			delete temp_comp2_node;
			comp2_node_pushed = false;
		}
		else {
			comp_comm_group_mgt_mgr->push_comp_node(temp_comp2_node);
			comp2_node = temp_comp2_node;
			comp2_node_pushed = true;
		}
	}

	interface1->get_fields_name(&interface1_fields_name);
	if (interface2 != NULL) {
		interface2->get_fields_name(&interface2_fields_name);
		EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, interface2_fields_name.size() == interface1_fields_name.size(), "Software error in Inout_interface_mgt::do_coupling_generation_between_interfaces_intra_one_component_model");
		for (i = 0; i < interface1_fields_name.size(); i ++) {
			for (j = 0; j < interface2_fields_name.size(); j ++)
				if (words_are_the_same(interface2_fields_name[j], interface1_fields_name[i]))
					break;
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, j < interface2_fields_name.size(), "Software error in Inout_interface_mgt::do_coupling_generation_between_interfaces_intra_one_component_model");
		}
	}
	
	if (interface2 != NULL)
		EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, interface1->get_interface_type() == COUPLING_INTERFACE_MARK_IMPORT && interface2->get_interface_type() == COUPLING_INTERFACE_MARK_EXPORT || interface1->get_interface_type() == COUPLING_INTERFACE_MARK_EXPORT && interface2->get_interface_type() == COUPLING_INTERFACE_MARK_IMPORT, "Software error in Inout_interface_mgt::do_coupling_generation_between_interfaces_intra_one_component_model");
    Coupling_connection *coupling_connection = new Coupling_connection(coupling_generator->apply_connection_id());
	if (interface1->get_interface_type() == COUPLING_INTERFACE_MARK_IMPORT) {
		strcpy(coupling_connection->dst_comp_full_name, comp1_node->get_full_name());
		strcpy(coupling_connection->dst_interface_name, interface1->get_interface_name());
		src_comp_interface_info.first = strdup(comp2_full_name);
		src_comp_interface_info.second = strdup(interface2_name);
		import_interface = interface1;
	}
	else {
		strcpy(coupling_connection->dst_comp_full_name, comp2_full_name);
		strcpy(coupling_connection->dst_interface_name, interface2_name);
		src_comp_interface_info.first = strdup(comp1_node->get_full_name());
		src_comp_interface_info.second = strdup(interface1->get_interface_name());
		import_interface = interface2;
	}
	coupling_connection->src_comp_interfaces.push_back(src_comp_interface_info);
	interfaces.push_back(interface1);
	if (interface2 != NULL)
		interfaces.push_back(interface2);
	if (import_interface != NULL)
		for (int i = 0; i < import_interface->get_num_coupling_procedures(); i ++)
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, !import_interface->get_field_connected_status(i), "Software error in Inout_interface_mgt::do_coupling_generation_between_interfaces_intra_one_component_model");
	for (int i = 0; i < interface1_fields_name.size(); i ++)
		coupling_connection->fields_name.push_back(strdup(interface1_fields_name[i]));
	coupling_connection->generate_a_coupling_procedure(has_frac_remapping);
	if (interface2 != NULL)
		interfaces.erase(interfaces.begin()+interfaces.size()-1);
	interfaces.erase(interfaces.begin()+interfaces.size()-1);

	if (comp2_node_pushed) {
