...
template <class T> void Runtime_trans_algorithm::unpack_segment_data(T *mpi_buf, T *field_data_buf, int segment_start, int segment_size, int field_2D_size, int total_dim_size_before_H2D, int total_dim_size_after_H2D, long field_size)
{
    int i, j, k, offset = 0;
	T *current_field_data_buf;
        ...
        for (int i = 0; i < num_transfered_fields; i ++) {
            ...
            if (fields_routers[i]->get_num_dimensions() == 0) {
				...
				if (fields_mem[i]->get_size_of_field() > 0) {
					...
					EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "field_total_dim_size_before_H2D and field_total_dim_size_after_H2D for grid \"%s\" are %d and %d", fields_mem[i]->get_field_data()->get_coord_value_grid()->get_grid_name(), field_total_dim_size_before_H2D[i], field_total_dim_size_after_H2D[i]);
				}				
            } 
        }		
...
void Runtime_trans_algorithm::receive_data_in_temp_buffer()
{
    ...
    if (empty_history_receive_buffer_index == -1) {
        ...
        for (int i = 0; i < num_transfered_fields; i ++) {
			if (for_halo_exchange)
				new_receive_fields_mem.push_back(fields_mem[i]);
			else new_receive_fields_mem.push_back(memory_manager->alloc_mem(fields_mem[i], BUF_MARK_DATA_TRANSFER, history_receive_fields_mem.size(), fields_mem[i]->get_data_type(), false));
        }
		if (for_halo_exchange)
