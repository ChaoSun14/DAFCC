...
bool H2D_remapping_wgt_file_info::match_H2D_remapping_wgt(Original_grid_info *src_original_grid, Original_grid_info *dst_original_grid)
{
    ...
    check_result = are_two_coord_arrays_same(original_grid_center_lon, this->src_center_lon, src_original_grid->get_H2D_sub_CoR_grid()->get_grid_size(), this->src_grid_size, false);
    ...
    check_result = are_two_coord_arrays_same(original_grid_center_lon, this->dst_center_lon, dst_original_grid->get_H2D_sub_CoR_grid()->get_grid_size(), this->dst_grid_size, false);
	delete [] original_grid_center_lon;
    if (!check_result) {
        clean();
        return false;
    ...
    check_result = are_two_coord_arrays_same(original_grid_center_lat, this->src_center_lat, src_original_grid->get_H2D_sub_CoR_grid()->get_grid_size(), this->src_grid_size, false);
    ...
    check_result = are_two_coord_arrays_same(original_grid_center_lat, this->dst_center_lat, dst_original_grid->get_H2D_sub_CoR_grid()->get_grid_size(), this->dst_grid_size, false);
	delete [] original_grid_center_lat;
    if (!check_result) {
        clean();
        return false;
...
{
	char full_default_wgt_file_name[NAME_STR_SIZE*2], src_H2D_sub_grid_name[NAME_STR_SIZE], dst_H2D_sub_grid_name[NAME_STR_SIZE];
	...
	if (src_original_grid->get_H2D_sub_CoR_grid() != NULL && algorithm_name != NULL) {	
		EXECUTION_REPORT(REPORT_ERROR, -1, src_original_grid->get_H2D_sub_grid_full_name(src_H2D_sub_grid_name), "Software error in H2D_remapping_wgt_file_mgt::search_H2D_remapping_weight");
		EXECUTION_REPORT(REPORT_ERROR, -1, dst_original_grid->get_H2D_sub_grid_full_name(dst_H2D_sub_grid_name), "Software error in H2D_remapping_wgt_file_mgt::search_H2D_remapping_weight");		
		sprintf(full_default_wgt_file_name, "%s/DEFAULT_WGT__%s__FROM__%s__TO__%s.nc", comp_comm_group_mgt_mgr->get_internal_remapping_weights_dir(), algorithm_name, src_H2D_sub_grid_name, dst_H2D_sub_grid_name);
...
void Remapping_setting::shrink(Original_grid_info *src_grid, Original_grid_info *dst_grid)
{
    ...
    if (src_grid->get_Time1D_sub_CoR_grid() == NULL)
