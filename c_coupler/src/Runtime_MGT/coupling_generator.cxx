...
void Coupling_connection::generate_data_transfer()
{
    ...
    for (int i = 0; i < src_fields_info.size(); i ++) { 
        if (dst_fields_info[i]->runtime_remapping_weights_under_V3D != NULL && dst_fields_info[i]->runtime_remapping_weights_under_V3D->get_src_decomp_info() != NULL) {
            strcpy(temp_dst_decomp_name, dst_fields_info[i]->runtime_remapping_weights_under_V3D->get_src_decomp_info()->get_decomp_name());
            dst_comp_id = dst_fields_info[i]->runtime_remapping_weights_under_V3D->get_src_decomp_info()->get_comp_id();
        }
        else {
...
bool Coupling_connection::exchange_grid(Comp_comm_group_mgt_node *sender_comp_node, Comp_comm_group_mgt_node *receiver_comp_node, const char *grid_name)
{
    ...
    if (original_grid_status == 0) {
        ...
        if (receiver_original_grid->get_bottom_field_variation_type() != bottom_field_variation_type)
            EXECUTION_REPORT(REPORT_ERROR, -1, receiver_original_grid->has_sigma_sub_grid(), "Software error in Coupling_connection::exchange_grid regarding bottom_field_variation_type");
...
void Coupling_connection::add_bottom_field_coupling_info(int field_connection_indx, Runtime_remapping_weights_container *remapping_weights_container, Remapping_setting *remapping_setting)
{
	Runtime_remapping_weights *V3D_remapping_weights = remapping_weights_container->get_runtime_remapping_weights_under_V3D();

	
	EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "add_bottom_field_coupling_info for the remapping weight from \"%s\" to \"%s\"", V3D_remapping_weights->get_src_original_grid()->get_grid_name(), V3D_remapping_weights->get_dst_original_grid()->get_grid_name());
	...
	if (V3D_remapping_weights->get_src_original_grid()->get_original_CoR_grid()->is_sigma_grid()) {		
		bottom_field_coupling_info->runtime_remapping_weights_container = new Runtime_remapping_weights_container(src_comp_node->get_comp_full_name(), dst_comp_node->get_comp_full_name(), original_grid_mgr->get_original_grid(V3D_remapping_weights->get_src_decomp_info()->get_grid_id()), original_grid_mgr->get_original_grid(V3D_remapping_weights->get_dst_decomp_info()->get_grid_id()), remapping_setting, V3D_remapping_weights->get_dst_decomp_info());
		...
		bottom_field_coupling_info->runtime_remapping_weights_container = remapping_weights_container;
		bottom_field_coupling_info->is_dynamic_bottom_field = V3D_remapping_weights->get_src_original_grid()->get_V3D_lev_field_variation_type() == BOTTOM_FIELD_VARIATION_DYNAMIC;
	}
    bottom_field_coupling_info->bottom_field_inst = V3D_remapping_weights->allocate_intermediate_V3D_grid_bottom_field();

	if (V3D_remapping_weights->get_src_original_grid()->get_original_CoR_grid()->is_sigma_grid())
	    bottom_field_coupling_info->H2D_runtime_remapping_weights = bottom_field_coupling_info->runtime_remapping_weights_container->get_runtime_remapping_weights_under_V3D();
    ...
    if (V3D_remapping_weights->get_parallel_remapping_weights() != NULL) {
        ...
        if (V3D_remapping_weights->get_dst_original_grid()->get_bottom_field_variation_type() == BOTTOM_FIELD_VARIATION_EXTERNAL) {
			EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "add_bottom_field_coupling_info for the remapping weight from \"%s\" to \"%s\": detect dst external surface field", V3D_remapping_weights->get_src_original_grid()->get_grid_name(), V3D_remapping_weights->get_dst_original_grid()->get_grid_name());
...
void Coupling_connection::generate_interpolation(bool has_frac_remapping)
{
    ...
    for (int i = 0; i < fields_name.size(); i ++) {
        ...
        src_fields_info[i]->runtime_remapping_weights_under_V3D = NULL;
        dst_fields_info[i]->runtime_remapping_weights_under_V3D = NULL;		
		src_fields_info[i]->runtime_remapping_weights_container = NULL;
		dst_fields_info[i]->runtime_remapping_weights_container = NULL;
        ...
        if (src_comp_node == dst_comp_node) {
			if (words_are_the_same(src_fields_info[i]->grid_name, dst_fields_info[i]->grid_name))
	            continue;
	        ...
	        if (dst_original_grid->get_original_CoR_grid()->is_similar_grid_with(src_original_grid->get_original_CoR_grid())) {
				EXECUTION_REPORT_LOG(REPORT_LOG, src_comp_node->get_comp_id(), true, "No data interpolation will be generated for similar grids \"%s\" and \"%s\"", src_fields_info[i]->grid_name, dst_fields_info[i]->grid_name);
	            continue;
	        }
        }
...
//        exchange_grid(dst_comp_node, src_comp_node, dst_fields_info[i]->grid_name);
        ...
        if (current_proc_id_dst_comp != -1) {
            dst_original_grid = original_grid_mgr->search_grid_info(dst_fields_info[i]->grid_name, comp_comm_group_mgt_mgr->search_global_node(dst_comp_full_name)->get_comp_id());
            src_original_grid = original_grid_mgr->search_grid_info(src_fields_info[i]->grid_name, comp_comm_group_mgt_mgr->search_global_node(src_comp_interfaces[0].first)->get_comp_id());
            if (src_original_grid->is_the_same_as_another_grid(dst_original_grid)) {
            ...
            if (dst_original_grid->has_sigma_sub_grid()) {
                EXECUTION_REPORT(REPORT_ERROR, dst_comp_node->get_comp_id(), dst_original_grid->get_bottom_field_variation_type() != BOTTOM_FIELD_VARIATION_UNSET, "Fail to generate an interpolation from component model \"%s\" to \"%s\": the surface field of the target grid \"%s\" that includes SIGMA or HYBRID vertical coordinate has not been specified. Please verify.", src_comp_interfaces[0].first, dst_comp_full_name, dst_original_grid->get_grid_name());
                if (dst_original_grid->get_bottom_field_variation_type() == BOTTOM_FIELD_VARIATION_EXTERNAL)
                    EXECUTION_REPORT(REPORT_ERROR, dst_comp_node->get_comp_id(), src_original_grid->has_sigma_sub_grid(), "Fail to generate an interpolation from component model \"%s\" to \"%s\": the target grid \"%s\" has an external surface field; at this time, the source grid \"%s\" should include SIGMA or HYBRID vertical coordinate but actually not. Please verify. ", src_comp_interfaces[0].first, dst_comp_full_name, dst_original_grid->get_grid_name(), src_original_grid->get_grid_name());
            }
			dst_fields_info[i]->runtime_remapping_weights_container = new Runtime_remapping_weights_container(src_comp_node->get_comp_full_name(), dst_comp_node->get_comp_full_name(), src_original_grid, dst_original_grid, &field_remapping_setting, decomps_info_mgr->search_decomp_info(dst_fields_info[i]->decomp_name, dst_comp_node->get_comp_id()));
			dst_fields_info[i]->runtime_remapping_weights_under_V3D = dst_fields_info[i]->runtime_remapping_weights_container->get_runtime_remapping_weights_under_V3D();
//            dst_fields_info[i]->runtime_remapping_weights = runtime_remapping_weights_mgr->search_or_generate_runtime_remapping_weights(src_comp_node->get_comp_full_name(), dst_comp_node->get_comp_full_name(), src_original_grid, dst_original_grid, &field_remapping_setting, decomps_info_mgr->search_decomp_info(dst_fields_info[i]->decomp_name, dst_comp_node->get_comp_id()));
			dst_fields_info[i]->runtime_remapping_weights = (Runtime_remapping_weights*)(1);
            if (src_original_grid->has_sigma_sub_grid() || src_original_grid->does_use_V3D_level_coord())
                add_bottom_field_coupling_info(i, dst_fields_info[i]->runtime_remapping_weights_container, &field_remapping_setting);
        }
    }
    ...
    if (current_proc_id_dst_comp != -1) 
        for (int i = 0; i < fields_name.size(); i ++)
            if (dst_fields_info[i]->runtime_remapping_weights_under_V3D == NULL || !dst_fields_info[i]->runtime_remapping_weights_under_V3D->get_src_original_grid()->is_H2D_grid())
                remapping_weights_index_table[i] = -1;
            else {
                for (j = 0; j < i; j ++)
                    if (dst_fields_info[i]->runtime_remapping_weights_under_V3D == dst_fields_info[j]->runtime_remapping_weights_under_V3D)
                        break;
    ...
    for (int i = 0; i < fields_name.size(); i ++) {
        ...
        if (remapping_weights_index_table[i] != i) {
            if (current_proc_id_src_comp != -1)
                src_fields_info[i]->runtime_remapping_weights_under_V3D = src_fields_info[remapping_weights_index_table[i]]->runtime_remapping_weights_under_V3D;    
        }
        else runtime_remapping_weights_mgr->transfer_runtime_remapping_weights(dst_fields_info[i]->runtime_remapping_weights_under_V3D, &(src_fields_info[i]->runtime_remapping_weights_under_V3D), dst_comp_node, src_comp_node);
    }
...
void Coupling_connection::read_fields_info_from_array(std::vector<Interface_field_info*> &fields_info, const char *array_buffer, long buffer_content_iter)
{
    while (buffer_content_iter > 0) {
		...
		field_info->runtime_remapping_weights_container = NULL;
		field_info->runtime_remapping_weights_under_V3D = NULL;
...
void Coupling_connection::write_field_info_into_array(Field_mem_info *field, char **array, long &buffer_max_size, long &buffer_content_size)
{
    ...
    else strcpy(tmp_string, grid_name);
...
void Coupling_connection::exchange_bottom_fields_info()
{
    ...
    for (int i = fields_name.size(); i < dst_fields_info.size(); i ++) {
        ...
        if (current_proc_id_dst_comp != -1) {
            dst_fields_info[i]->runtime_remapping_weights = (Runtime_remapping_weights*) 1;
            dst_fields_info[i]->runtime_remapping_weights_under_V3D = dst_bottom_fields_coupling_info[dst_fields_info[i]->bottom_field_indx]->H2D_runtime_remapping_weights;			
            dst_fields_info[i]->runtime_remapping_weights_container = dst_bottom_fields_coupling_info[dst_fields_info[i]->bottom_field_indx]->runtime_remapping_weights_container;			
        }
    }
...
//                        EXECUTION_REPORT(REPORT_ERROR, host_comp_id, comp_comm_group_mgt_mgr->search_global_node(full_name) != NULL, "When setting the coupling connection configuration of the import interface \"%s\" in the XML file \"%s\", the full component name (\"%s\") is wrong. Please verify the XML file arround the line number %d.", interface_name, XML_file_name, full_name, line_number);
                    ...
                    if (full_name == NULL)
						...
						if (strlen(ignored_full_name) > 0) {
							sprintf(temp_full_name, "%s@%s", ignored_full_name, full_name);
							producer_info.first = strdup(temp_full_name);
							EXECUTION_REPORT_LOG(REPORT_LOG, host_comp_id, true, "component full name \"%s\" will be added to the comp_full_name \"%s\" to generate \"%s\"", ignored_full_name, full_name, temp_full_name);
						}
...
Import_interface_configuration::Import_interface_configuration(int host_comp_id, const char *comp_full_name, const char *interface_name, TiXmlElement *interface_element, const char *XML_file_name, Inout_interface_mgt *all_interfaces_mgr, bool check_comp_existence, const char *ignored_full_name)
{
    ...
    for (TiXmlNode *redirection_element_node = interface_element->FirstChild(); redirection_element_node != NULL; redirection_element_node = redirection_element_node->NextSibling()) {
        ...
        if (!is_XML_setting_on(interface_ptr->get_comp_id(), redirection_element, XML_file_name, "the status of some coupling connection configurations for an import interface", "import interface configuration file"))
            continue;
        import_directions.push_back(new Import_direction_setting(host_comp_id, this, comp_full_name, interface_name, redirection_element, XML_file_name, fields_name, fields_count, check_comp_existence, ignored_full_name));
    }
...
Component_import_interfaces_configuration::Component_import_interfaces_configuration(int host_comp_id, const char *comp_full_name, Inout_interface_mgt *interface_mgr, bool check_comp_existence, int comp_id_ignore_full_name)
{
    char XML_file_name[NAME_STR_SIZE], ignored_full_name[NAME_STR_SIZE];
    int line_number;


	ignored_full_name[0] = '\0';
    strcpy(this->comp_full_name, comp_full_name);
	if (comp_id_ignore_full_name != -1 && !words_are_the_same(comp_full_name, comp_comm_group_mgt_mgr->search_global_node(comp_id_ignore_full_name)->get_full_name()) && comp_comm_group_mgt_mgr->search_global_node(comp_id_ignore_full_name)->is_real_component_model()) {
		char real_comp_long_name[NAME_STR_SIZE];
		strcpy(ignored_full_name, comp_comm_group_mgt_mgr->search_global_node(comp_id_ignore_full_name)->get_full_name());
		EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, strlen(comp_full_name) > strlen(ignored_full_name) && strncmp(comp_full_name, ignored_full_name, strlen(ignored_full_name)) == 0, "Software error in Component_import_interfaces_configuration::Component_import_interfaces_configuration");
		strcpy(real_comp_long_name, comp_full_name+strlen(ignored_full_name)+1);
		EXECUTION_REPORT_LOG(REPORT_LOG, host_comp_id, true, "The full name of the component model \"%s\" will be ignored when searching (%s)", ignored_full_name, real_comp_long_name);
		sprintf(XML_file_name, "%s/all/coupling_connections/%s.coupling_connections.xml", comp_comm_group_mgt_mgr->get_config_root_dir(), real_comp_long_name);
	}
	else sprintf(XML_file_name, "%s/all/coupling_connections/%s.coupling_connections.xml", comp_comm_group_mgt_mgr->get_config_root_dir(), comp_full_name);
    ...
    for (TiXmlNode *interface_XML_element_node = root_XML_element->FirstChild(); interface_XML_element_node != NULL; interface_XML_element_node = interface_XML_element_node->NextSibling()) {
        ...
        import_interfaces_configuration.push_back(new Import_interface_configuration(host_comp_id, comp_full_name, import_interface->get_interface_name(), interface_XML_element, XML_file_name, interface_mgr, check_comp_existence, ignored_full_name));
    }
...
void Coupling_generator::generate_coupling_procedures_common(int API_id, MPI_Comm comm, bool is_overall_generation, bool is_internal_generation, int comp_id_ignore_full_name, const char *annotation)
{
    ...
    if (current_proc_local_id == 0) {
        ...
        for (int i = 0; i < all_comp_fullnames_for_coupling_generation.size(); i ++) {
            ...
            Component_import_interfaces_configuration *comp_import_interfaces_config = new Component_import_interfaces_configuration(local_comp_node->get_comp_id(), all_comp_fullnames_for_coupling_generation[i], all_interfaces_mgr, is_overall_generation, comp_id_ignore_full_name);
...
void Coupling_generator::generate_coupling_procedures_internal(int comp_id, bool family_generation, bool is_internal_generation, int ancestor_id_for_ignoring_comp_name, const char *annotation)
{
    char *temp_array_buffer = NULL;
    long current_array_buffer_size, max_array_buffer_size;
    ...
    if (family_generation) {
		...
		for (int i = 0; i < all_comp_fullnames_for_coupling_generation.size(); i ++)
			EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "family coupling generation include \"%s\"", all_comp_fullnames_for_coupling_generation[i]);
        generate_coupling_procedures_common(API_ID_COUPLING_GEN_FAMILY, comp_comm_group_mgt_mgr->get_comm_group_of_local_comp(comp_id, "in Coupling_generator::generate_coupling_procedures"), (comp_id & TYPE_ID_SUFFIX_MASK)==0, is_internal_generation, ancestor_id_for_ignoring_comp_name, annotation);
    }
    else {
        ...
        generate_coupling_procedures_common(API_ID_COUPLING_GEN_INDIVIDUAL, comp_comm_group_mgt_mgr->get_comm_group_of_local_comp(comp_id, "in Coupling_generator::generate_coupling_procedures"), (comp_id & TYPE_ID_SUFFIX_MASK)==0, is_internal_generation, -1, annotation);
    }
}
...
void Coupling_generator::do_external_coupling_generation(int API_id, const char *annotation)
{
    ...
    if (all_comp_fullnames_for_coupling_generation.size() == 1) {
        generate_coupling_procedures_internal(all_comp_nodes[0]->get_comp_id(), individual_or_family_generation[0] == 2, false, -1, annotation);
    ...
    generate_coupling_procedures_common(API_id, union_comm, false, false, -1, annotation);
