...
Runtime_cumulate_average_algorithm::Runtime_cumulate_average_algorithm(Connection_coupling_procedure *coupling_procedure, Field_mem_info *field_src, Field_mem_info *field_dst)
{
	...
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, field_src->get_decomp_id() == field_dst->get_decomp_id(), "Software error in Runtime_cumulate_average_algorithm::Runtime_cumulate_average_algorithm");
	if (field_src->get_field_data()->get_coord_value_grid() != field_dst->get_field_data()->get_coord_value_grid()) {
		...
		if (field_src->get_num_chunks() > 0)
			cumulate_average_field->mem_info_src = memory_manager->alloc_mem(field_dst, BUF_MARK_AVERAGED_BACKUP, 0, NULL, false);
	}
