                                       ...
                                       bool is_remap_operator_regridding)
{
	...
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, remap_src_grid->is_subset_of_grid(field_data_grid), "Software error in Remap_grid_class::Remap_grid_class for remapping: \"%s\" (%lx)  vs  \"%s\" (%lx)", field_data_grid->get_grid_name(), field_data_grid, remap_src_grid->get_grid_name(), remap_src_grid);
    ...
    for (j = 0; j < num_leaf_grids_field_data; j ++) 
		for (i = 0; i < num_leaf_grids_src; i ++)
            if (words_are_the_same(leaf_grids_field_data[j]->get_coord_label(), leaf_grids_src[i]->get_coord_label())) {
				EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, words_are_the_same(leaf_grids_field_data[j]->get_grid_name(), leaf_grids_src[i]->get_grid_name()), "Software error in Remap_grid_class::Remap_grid_class for remapping");
				for (k = 0; k < num_leaf_grids_dst; k ++) 
					if (words_are_the_same(leaf_grids_src[i]->get_coord_label(), leaf_grids_dst[k]->get_coord_label())) {
	                	leaf_grids_field_data[j] = leaf_grids_dst[k];
	                	break;
            		}
				EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, k < num_leaf_grids_dst, "Software error in Remap_grid_class::Remap_grid_class for remapping");
				break;
            }
                                                ...
                                                int num_runtime_mask_sub_grids)
{
    ...
    char new_grid_name[NAME_STR_SIZE];
    int i, j, k;
...
bool Remap_grid_class::is_level_V3D_coord_trigger_field_updated()
{
	...
	if (level_V3D_coord_dynamic_trigger_field->get_grid_data_field()->required_data_size > 0) {
	    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, level_V3D_coord_trigger_field->get_grid_data_field()->data_buf != NULL, "C-Coupler error1 in Remap_grid_class::is_level_V3D_coord_trigger_field_updated");
	    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, level_V3D_coord_dynamic_trigger_field->get_grid_data_field()->data_buf != NULL, "C-Coupler error1 in Remap_grid_class::is_level_V3D_coord_trigger_field_updated");		
	}
...
void Remap_grid_class::set_level_V3D_coord_dynamic_trigger_field(Remap_grid_data_class *value_field)
{
	...
	EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "Set level_V3D_coord_dynamic_trigger_field for grid \"%s\"", grid_name);
...
bool Remap_grid_class::is_the_same_grid_with(Remap_grid_class *another_grid) 
{
	...
	if (this->grid_size != another_grid->grid_size)
		return false;
...
bool Remap_grid_class::have_the_same_dimensions_with(Remap_grid_class *another_grid) 
{
    int num_leaf_grids_this, i;
    Remap_grid_class *leaf_grids_this[128];
    ...
    get_leaf_grids(&num_leaf_grids_this, leaf_grids_this, this);
	for (i = 0; i < num_leaf_grids_this; i ++)
		if (!another_grid->has_grid_coord_label(leaf_grids_this[i]->get_coord_label()))
			return false;
...
void Remap_grid_class::get_sized_sub_grids(int *num_sized_sub_grids, Remap_grid_class **sized_sub_grids) 
{
    ...
    *num_sized_sub_grids = 0;
    for (i = 0; i < num_leaf_grids; i ++) {
        ...
        EXECUTION_REPORT(REPORT_ERROR, -1, sized_super_grid != NULL, "remap software error1 in function get_sized_sub_grids %s\n", leaf_grids[i]->get_grid_name());
    ...
    for (i = 0; i < *num_sized_sub_grids; i ++) {
        ...
        if (original_grid == NULL)
            EXECUTION_REPORT(REPORT_ERROR, -1, is_superset_of_grid(sized_sub_grids[i]), "remap software error3 in function get_sized_sub_grids: \"%s\" \"%s\" (%lx)\n", grid_name, sized_sub_grids[i]->get_grid_name(), sized_sub_grids[i]);
...
void Remap_grid_class::check_center_vertex_values_consistency_2D()
{
    ...
    for (i = 0; i < grid_size; i ++) {
        ...
        if (!is_point_in_2D_cell(center_coord1_value,
            ...
            EXECUTION_REPORT(REPORT_ERROR, -1, false, "At the No. %d point of the grid \"%s\", the center coordinate values (lon=%lf,lat=%lf) are not consistent with vertex coordinate values (%s): each center point must be surrounded by the polygon determined by the corresponding vertex points.", i+1, grid_name, center_coord1_value, center_coord2_value, error_string1);
        }
    }
}
                                                             ...
                                                             bool require_full_grid_mask)
{
    ...
    if ((*num_mask_sub_grids) == 1 && !require_full_grid_mask) {
...
bool Remap_grid_class::has_V3D_sub_grid()
{
	return has_grid_coord_label(COORD_LABEL_LON) && has_grid_coord_label(COORD_LABEL_LAT) && has_grid_coord_label(COORD_LABEL_LEV);
}


Remap_grid_class *Remap_grid_class::get_V3D_sub_grid()
{
    if (num_dimensions < 3)
		return NULL;

	if (num_dimensions == 3 && has_V3D_sub_grid())
		return this;

    for (int i = 0; i < sub_grids.size(); i ++)
        if (sub_grids[i]->get_V3D_sub_grid() != NULL)
            return sub_grids[i]->get_V3D_sub_grid();
...
void Remap_grid_class::get_total_dim_size_before_and_after_H2D(int &total_dim_size_before_H2D, int &total_dim_size_after_H2D)
{
	int i, sub_sphere_grid_index_iter = -1, num_sized_grids;
	Remap_grid_class *sized_grids[256]; 
	

	EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "Remap_grid_class::get_total_dim_size_before_and_after_H2D: for grid \"%s\"\n", grid_name);	
	get_sized_sub_grids(&num_sized_grids, sized_grids);	
	for (i = 0; i < num_sized_grids; i ++)
		EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "sized grid %d is \"%s\" (%d)", i, sized_grids[i]->get_grid_name(), sized_grids[i]->get_grid_size());
	for (i = 0, total_dim_size_before_H2D = 1, total_dim_size_after_H2D = 1; i < num_sized_grids; i ++)
		if (sized_grids[i]->has_grid_coord_label(COORD_LABEL_LON) || sized_grids[i]->has_grid_coord_label(COORD_LABEL_LAT)) {
			if (sub_sphere_grid_index_iter != -1)
				EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, sub_sphere_grid_index_iter == i - 1, "software error in Remap_grid_class::get_total_dim_size_before_and_after_H2D");
			sub_sphere_grid_index_iter = i;
		}
		else if (sub_sphere_grid_index_iter == -1)
			total_dim_size_before_H2D *= sized_grids[i]->get_grid_size();
		else total_dim_size_after_H2D *= sized_grids[i]->get_grid_size();
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, total_dim_size_before_H2D > 0 && total_dim_size_after_H2D > 0, "software error in Remap_grid_class::get_total_dim_size_before_and_after_H2D");
}


Remap_grid_class *Remap_grid_class::get_ordered_similar_grid_under_V3D()
{
	Remap_grid_class *new_sub_grids[256], *ordered_grid, *existing_grid;
	char new_grid_name[NAME_STR_SIZE];
	int num_new_sub_grids = 0;
	

	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, num_dimensions <= 3, "Software error in Remap_grid_class::get_ordered_similar_grid_under_V3D");

	if (num_dimensions < 3)
		return this;

	sprintf(new_grid_name, "ORDERED_%s", grid_name);
	existing_grid = remap_grid_manager->search_remap_grid_with_grid_name(new_grid_name);
	if (existing_grid != NULL)
		return existing_grid;

	for (int i = 0; i < sub_grids.size(); i ++)
		if (sub_grids[i]->has_grid_coord_label(COORD_LABEL_LAT) || sub_grids[i]->has_grid_coord_label(COORD_LABEL_LON))
			new_sub_grids[num_new_sub_grids++] = sub_grids[i];
	for (int i = 0; i < sub_grids.size(); i ++)
		if (sub_grids[i]->has_grid_coord_label(COORD_LABEL_LEV))
			new_sub_grids[num_new_sub_grids++] = sub_grids[i];
	ordered_grid = new Remap_grid_class(new_grid_name, num_new_sub_grids, new_sub_grids, 0);

	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, ordered_grid->is_similar_grid_with(this), "Software error in Remap_grid_class::get_ordered_similar_grid_under_V3D");
	remap_grid_manager->add_temp_grid(ordered_grid);
	return ordered_grid;
}


bool Remap_grid_class::get_is_empty_grid()
{
	for (int i = 0; i < sub_grids.size(); i ++)
		if (sub_grids[i]->get_is_empty_grid())
			return true;
