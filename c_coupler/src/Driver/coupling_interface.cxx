...
#ifdef LINK_WITHOUT_UNDERLINE
extern "C" void start_registering_ccpl_ensemble_set
#else
extern "C" void start_registering_ccpl_ensemble_set_
#endif
(const char *comps_ensemble_set_name, int *comp_id, int *num_comps, int *size_comps_names, const char *annotation)
{
    EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "Start to run the API \"CCPL_register_ensemble_set_of_components\" to register the ensemble set named \"%s\"", comps_ensemble_set_name);    	
    check_for_component_registered(*comp_id, API_ID_COMP_MGT_REG_ENSEMBLE_SET, annotation, false);
	EXECUTION_REPORT(REPORT_ERROR, *comp_id, *num_comps > 0, "ERROR happens when calling the API \"CCPL_register_ensemble_set_of_components\" at the model code with the annotation \"%s\": the input parameter of \"num_comps\" (currently is %d) cannot be smaller than 1. Please verify.", annotation, *num_comps);
	EXECUTION_REPORT(REPORT_ERROR, *comp_id, *num_comps == 1, "Software error happens when calling the API \"CCPL_register_ensemble_set_of_components\" at the model code with the annotation \"%s\": the input parameter of \"num_comps\" (currently is %d) must be 1 currently. Please contact Dr. Li Liu for special support.", annotation, *num_comps);	
	EXECUTION_REPORT(REPORT_ERROR, *comp_id, *size_comps_names >= *num_comps, "ERROR happens when calling the API \"CCPL_register_ensemble_set_of_components\" at the model code with the annotation \"%s\": the number of full names in the input parameter of \"comps_common_full_names\" (currently is %d) is smaller than the input parameter of \"num_comps\" (currently is %d). Please verify.", annotation, *size_comps_names, *num_comps);
	comp_comm_group_mgt_mgr->clear_temp_comps_name();
}


#ifdef LINK_WITHOUT_UNDERLINE
extern "C" void add_common_comp_name_for_ensemble_set
#else
extern "C" void add_common_comp_name_for_ensemble_set_
#endif
(const char *comp_name, const char *annotation)
{
	comp_comm_group_mgt_mgr->add_temp_comp_name(comp_name);
}


#ifdef LINK_WITHOUT_UNDERLINE
extern "C" void register_ccpl_ensemble_set_of_comps
#else
extern "C" void register_ccpl_ensemble_set_of_comps_
#endif
(const char *comps_ensemble_set_name, int *comps_ensemble_set_id, int *comp_id, int *ensemble_size, const char *annotation)
// ensemble_size can be got from parent_ensemble_set_id
{	
	*comps_ensemble_set_id = comp_comm_group_mgt_mgr->register_ensemble_set_of_comps(comps_ensemble_set_name, *comp_id, *ensemble_size, annotation);
    components_time_mgrs->clone_parent_comp_time_mgr(*comps_ensemble_set_id, *comp_id, annotation);
    EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "Finish the API \"CCPL_register_ensemble_set_of_components\" to register the ensemble set named \"%s\"", comps_ensemble_set_name);    	
}
...
(int *comp_id, int *grid_id, const char *grid_name, int *dim_size, const char *annotation)
{
    ...
    *grid_id = original_grid_mgr->register_V1D_grid_via_data(API_ID_GRID_MGT_REG_NORMAL_1D_GRID_NO_DATA, *comp_id, grid_name, 0, NULL, *dim_size, 0.0, NULL, NULL, annotation);
...
(int *grid_id, int *decomp_id, int *chunk_index, const char *label, const char *data_type, int *array_size, char *grid_data, const char *annotation)
{
    ...
    original_grid_mgr->get_original_grid(*grid_id)->get_grid_data(*decomp_id, *chunk_index, label, data_type, *array_size, grid_data, annotation, API_label);
}
...
(int *decomp_id, int *grid_id, int *num_local_cells, int *array_size, const int *local_cells_global_indx, const char *decomp_name, const char *annotation)
{
    ...
    *decomp_id = decomps_info_mgr->register_H2D_parallel_decomposition(decomp_name, *grid_id, *num_local_cells, local_cells_global_indx, 0, NULL, annotation);
...
(int *decomp_id, int *grid_id, int *num_chunks, const int *chunks_size, const int *local_cells_global_indx, int *size_chunks_size_array, int *size_global_indexes_array, const char *decomp_name, const char *annotation)
{
    ...
    *decomp_id = decomps_info_mgr->register_H2D_parallel_decomposition(decomp_name, *grid_id, num_all_local_cells, local_cells_global_indx, *num_chunks, chunks_size, annotation);
...
#endif
(int *instance_id, const char *inst_name, const char *procedures_name, const char *ptype, int *host_comp_id, const char *dl_name, int *process_active, int *size_controls, int *size_decomps, int *size_grids, int *size_field_inst, int *size_timers, 
 int *control_vars, int *decomp_ids, int *grid_ids, int *field_inst_ids, int *timer_ids, const char *annotation)
{
	...
	*instance_id = external_procedures_mgr->initialize_external_procedures_inst(inst_name, procedures_name, ptype, *host_comp_id, dl_name, *process_active, *size_controls, *size_decomps, *size_grids, *size_field_inst, *size_timers, control_vars, decomp_ids, grid_ids, field_inst_ids, timer_ids, annotation);	
...
#ifdef LINK_WITHOUT_UNDERLINE
extern "C" void get_ccpl_external_procedures_num_grid_decomps
#else
extern "C" void get_ccpl_external_procedures_num_grid_decomps_
#endif
(int *num_grid_decomps, int *instance_id, const char *annotation)
{
	EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "API CCPL_external_procedures_para_get_num_grid_decomps begins");
	check_for_ccpl_managers_allocated(API_ID_EXTERNAL_PROC_INST_GET_NUM_GRID_DECOMPS, annotation);
	*num_grid_decomps = external_procedures_mgr->get_instance_num_grid_decomps(*instance_id, annotation);
	EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "API CCPL_external_procedures_para_get_num_grid_decomps ends");
}
...
#endif
(int *grid_id, int *instance_id, int *grid_index, const char *annotation)
{
	EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "API CCPL_external_procedures_para_get_grid_ID begins");
	check_for_ccpl_managers_allocated(API_ID_EXTERNAL_PROC_INST_GET_GRID_ID, annotation);
	*grid_id = external_procedures_mgr->get_instance_grid_id(*instance_id, *grid_index, annotation);
	EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "API CCPL_external_procedures_para_get_grid_ID ends");
}
...
#endif
(int *decomp_id, int *instance_id, int *decomp_index, const char *annotation)
{
	EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "API CCPL_external_procedures_para_get_decomp_ID begins");
	check_for_ccpl_managers_allocated(API_ID_EXTERNAL_PROC_INST_GET_DECOMP_ID, annotation);
	*decomp_id = external_procedures_mgr->get_instance_decomp_id(*instance_id, *decomp_index, annotation);
	EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "API CCPL_external_procedures_para_get_decomp_ID ends");
}
...
#ifdef LINK_WITHOUT_UNDERLINE
extern "C" void get_ccpl_ensemble_info
#else
extern "C" void get_ccpl_ensemble_info_
#endif
(int *ensemble_member_nums, int *ensemble_member_id, char *argv)
{
    char ens_id_str;
    char *pos;
    int  ierr;

    pos=strstr(argv, "CCPL_ensemble_");
    if (pos==NULL) 
        EXECUTION_REPORT(REPORT_ERROR, -1, "Error happens when calling the API \"CCPL_get_ensemble_info\" to get the ensemble info: the input ensemble info should start with \"CCPL_ensemble_\"");
    ierr=sscanf(argv, "CCPL_ensemble_%d_%d", ensemble_member_nums, ensemble_member_id);
    if (ierr==0) 
        EXECUTION_REPORT(REPORT_ERROR, -1, "Error happens when calling the API \"CCPL_get_ensemble_info\" to get the ensemble info: the input ensemble info should take tht form of CCPL_ensemble_[ensemble_nums]_[ensemble_member_id]");
    if (*ensemble_member_nums<=0 || *ensemble_member_nums>99) 
        EXECUTION_REPORT(REPORT_ERROR, -1, "Error happens when calling the API \"CCPL_get_ensemble_info\" to get the ensemble info: the ensemble numbers should >= 1 and <= 99");
    if (*ensemble_member_id<=0 || *ensemble_member_id>*ensemble_member_nums) 
        EXECUTION_REPORT(REPORT_ERROR, -1, "Error happens when calling the API \"CCPL_get_ensemble_info\" to get the ensemble info: the ensemble member ID should >= 1 and <= ensemble numbers");
    EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "Get the ensemble info: ensemble_numbers=%d, ensemble_member_id=%d", *ensemble_member_nums, *ensemble_member_id);
}

#ifdef LINK_WITHOUT_UNDERLINE
extern "C" void change_to_ensemble_dir
#else
extern "C" void change_to_ensemble_dir_
#endif
(int *comp_id, int *ensemble_member_id)
{
    char *work_dir_before, work_dir_after[NAME_STR_SIZE];
    char fmt_id[3];
    int ierr;
    if ((work_dir_before = getcwd(NULL, NULL)) == NULL){
        EXECUTION_REPORT(REPORT_ERROR, -1, "Error happens when calling the API \"CCPL_change_to_ensemble_dir\" to get the current work directory");
    }
    else {
        EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "before directory is \"%s\"", work_dir_before);
        if (*ensemble_member_id<10){
            sprintf(fmt_id, "%02d", *ensemble_member_id);
        }
        else {
            sprintf(fmt_id, "%d", *ensemble_member_id);
        }
        EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "ensemble_member_id is \"%d\"", *ensemble_member_id);
        char comp_name[NAME_STR_SIZE];
        //if (sscanf(comp_comm_group_mgt_mgr->search_global_node(*comp_id)->get_comp_name(), "%[^_]", comp_name) != 1) EXECUTION_REPORT(REPORT_ERROR, -1, false, "ERROR happens when calling the API \"CCPL_ensemble_procedures_inst_init\" to get the ensemble id of current process,please make sure there are no \"_\" used in the component name");
        if (*ensemble_member_id>0 && *ensemble_member_id<=99){
            //sprintf(new_dir, "%s/run/%s/%s/data", root_working_dir, new_comp->get_comp_type(), new_comp->get_comp_name());
            //sprintf(work_dir_after, "%s/run/ensemble_%s/all", work_dir_before, fmt_id); Fgoals-g2
            sprintf(work_dir_after, "%s/run/ensemble_%s/%s/%s/data", comp_comm_group_mgt_mgr->get_root_working_dir(), fmt_id, comp_comm_group_mgt_mgr->get_global_node_of_local_comp(*comp_id, true, "get_ccpl_comp_type")->get_comp_type(), comp_comm_group_mgt_mgr->search_global_node(*comp_id)->get_comp_name());
            EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "after directory is \"%s\"", work_dir_after);
            ierr=chdir(work_dir_after);
            if (ierr==-1) EXECUTION_REPORT(REPORT_ERROR, -1, "Error happens when calling the API \"CCPL_change_to_ensemble_dir\" to change the ensemble work directory to \"%s\"", work_dir_after);
            EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "Change the ensemble work directory to \"%s\"", getcwd(NULL, NULL));
        }
        else {
            EXECUTION_REPORT(REPORT_ERROR, -1, "Error happens when calling the API \"CCPL_change_to_ensemble_dir\" to change the work directory: the input \"ensemble_member_id\" should > 0 and <= 99");
        }
   }
}

#ifdef LINK_WITHOUT_UNDERLINE
extern "C" void change_to_speci_dir
#else
extern "C" void change_to_speci_dir_
#endif
(const char *speci_dir)
{
    char dir_after[NAME_STR_SIZE], *dir_before;
    char API_label[NAME_STR_SIZE];
    int ierr; 
    dir_before = getcwd(NULL, NULL);
    EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "before directory is \"%s\"", dir_before);
    sprintf(dir_after, "%s/%s", comp_comm_group_mgt_mgr->get_root_working_dir(), speci_dir);
    EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "input directory is \"%s\"", speci_dir);
    //sprintf(dir_after, "%s/../../../../../%s", dir_before, speci_dir);
    EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "the specified directory is \"%s\"", dir_after);
    ierr=chdir(dir_after);
    if (ierr==-1) EXECUTION_REPORT(REPORT_ERROR, -1, "Error happens when calling the API \"%s\" to change the specified directory to \"%s\"", API_label, dir_after);
    EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "Change the ensemble work directory to \"%s\"", getcwd(NULL, NULL));
}
...
#endif
(int *instance_id, const char *inst_name, int *set_comp_id, int *member_comp_id, int *size_field_inst, int *size_grids, int *size_decomps, int *size_timers, int *size_controls,   
 int *field_inst_ids, int *grid_ids, int *decomp_ids, int *timer_ids, int *control_vars, const char *annotation)
{
    ...
    check_for_component_registered(*set_comp_id, API_ID_ENSEMBLE_PROC_INST_INIT, annotation, false);
    ...
    *instance_id = ensemble_procedures_mgr->initialize_ensemble_procedures_inst(inst_name, *set_comp_id, *member_comp_id, *size_field_inst, *size_grids, *size_decomps, *size_timers, 
        *size_controls, field_inst_ids, grid_ids, decomp_ids, timer_ids, control_vars, annotation); 
...
#endif
(int *instance_id, int *do_advance_time, int *chunk_index, const char *annotation)
{
    ...
    if (*do_advance_time == 0) ensemble_procedures_mgr->get_procedures_inst(*instance_id, API_ID_ENSEMBLE_PROC_INST_RUN, annotation)->run(false,*chunk_index, annotation);
    if (*do_advance_time == 1) ensemble_procedures_mgr->get_procedures_inst(*instance_id, API_ID_ENSEMBLE_PROC_INST_RUN, annotation)->run(true,*chunk_index, annotation);
