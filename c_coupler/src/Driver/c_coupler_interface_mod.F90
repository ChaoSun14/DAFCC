...
!*************************************************************
 ...
 MODULE CCPL_interface_mod
   ...
   public :: CCPL_abort
   public :: CCPL_get_ensemble_info
   public :: CCPL_change_to_ensemble_dir
   public :: CCPL_change_to_speci_dir
   public :: CCPL_ensemble_procedures_inst_init
   public :: CCPL_ensemble_procedures_inst_run
...
! 
 ...
 SUBROUTINE CCPL_get_comm_array_min_max_value_pos_double(comm, min_max_hint, values, pos1, pos2, pos3, pos4, annotation)
  ...
  SUBROUTINE CCPL_reduce_sum_array_real16(input_data, output_data, comm, root_proc, annotation)
    ...
    allocate(reduce_buf_real16(num_data*local_num_proc*2))
    if (root_proc == -1) then
       ...
       call mpi_gather(input_data,num_data*2,MPI_REAL8,reduce_buf_real16,num_data*2,MPI_REAL8,root_proc,comm,ierr)
       if (current_proc == root_proc) then
          do k=1,num_data
             output_data(k) = 0.0
             do i=1,local_num_proc
                output_data(k) = output_data(k) + reduce_buf_real16((i-1)*num_data+k)
...
#if ( defined  NO_MPI_REAL16 )
    call MPI_COMM_SIZE(comm, local_num_proc, ierr)
    allocate(reduce_buf_real16(local_num_proc*2))
    if (root_proc == -1) then
       call mpi_allgather(input_data,2,MPI_REAL8,reduce_buf_real16,2,MPI_REAL8,comm,ierr)
       output_data = 0.0
       do i=1,local_num_proc
          output_data = output_data + reduce_buf_real16(i)
       end do
    else
       call mpi_gather(input_data,2,MPI_REAL8,reduce_buf_real16,2,MPI_REAL8,root_proc,comm,ierr)
       if (current_proc == root_proc) then
          output_data = 0.0
          do i=1,local_num_proc
             output_data = output_data + reduce_buf_real16(i)
...
if (do_abort) do_abort_int = 1
   ...
   SUBROUTINE CCPL_get_ensemble_info(ensemble_nums, ensemble_id)

     implicit none
     integer, intent(out) :: ensemble_nums, ensemble_id
     integer              :: m, ierr
     character *2048      :: ccpl_ens_index
     m = iargc()
     if ( m > 1) then
        call GETARG(m, ccpl_ens_index) !ccpl_ens_index = CCPL_ensemble_*
        call get_ccpl_ensemble_info(ensemble_nums, ensemble_id, ccpl_ens_index)             
     else
        ensemble_nums = 0
        ensemble_id = 0 
     end if

   END SUBROUTINE CCPL_get_ensemble_info

   SUBROUTINE CCPL_change_to_ensemble_dir(comp_id, ensemble_id)

     implicit none
     integer, intent(in) :: comp_id, ensemble_id

     call change_to_ensemble_dir(comp_id, ensemble_id)

   END SUBROUTINE CCPL_change_to_ensemble_dir

   SUBROUTINE CCPL_change_to_speci_dir(specified_dir)

     implicit none
     character(len=*), intent(in)   :: specified_dir
     print*, "the specified directory is ", trim(specified_dir)//char(0)

     call change_to_speci_dir(trim(specified_dir)//char(0))

   END SUBROUTINE CCPL_change_to_speci_dir

   INTEGER FUNCTION CCPL_ensemble_procedures_inst_init(set_comp_id, member_comp_id, inst_name, &
                           field_inst_ids, grid_ids, decomp_ids, timers_id, control_vars, annotation)
                    
   implicit none
   character(len=*), intent(in)                :: inst_name
   integer,          intent(in)                :: set_comp_id, member_comp_id
   integer, target,  intent(in)                :: field_inst_ids(:)
   integer, target,  intent(in), optional      :: grid_ids(:), decomp_ids(:) 
   integer, target,  intent(in), optional      :: timers_id(:), control_vars(:) 
   character(len=*), intent(in), optional      :: annotation
   character *2048                             :: local_annotation
   integer, dimension(:), pointer              :: tmp_control_vars, tmp_grid_ids, tmp_decomp_ids
   integer, dimension(:), pointer              :: tmp_timers_id, tmp_int_array 
   integer                                     :: size_controls, size_grids, size_decomps
   integer                                     :: size_field_inst, size_timers, instance_id
   ...
   size_field_inst = size(field_inst_ids)
   call initialize_CCPL_ensemble_procedures_inst_init(instance_id, trim(inst_name)//char(0), set_comp_id, member_comp_id, &
          size_field_inst, size_grids, size_decomps, size_timers, size_controls, field_inst_ids, tmp_grid_ids, tmp_decomp_ids, &
          tmp_timers_id, tmp_control_vars, trim(local_annotation)//char(0))

   deallocate(tmp_int_array)

   CCPL_ensemble_procedures_inst_init = instance_id

   END FUNCTION CCPL_ensemble_procedures_inst_init


   SUBROUTINE CCPL_ensemble_procedures_inst_run(instance_id, do_advance_time, chunk_index, annotation)
     implicit none
     integer,          intent(in)                :: instance_id, chunk_index, do_advance_time
     character(len=*), intent(in), optional      :: annotation
     character *2048                             :: local_annotation
     
     local_annotation = ""
     if (present(annotation)) local_annotation = annotation
     call run_ccpl_ensemble_procedures_inst(instance_id, do_advance_time, chunk_index, trim(local_annotation)//char(0))

   END SUBROUTINE CCPL_ensemble_procedures_inst_run
